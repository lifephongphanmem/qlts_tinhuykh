...
$(document).on("click", ".open-AddBookDialog", function (e) {	
	...
	$("#namsudungcs").val(myBookId[5]);	
	$("#phantramhaomoncs").val(myBookId[6]);	
 ...
 <script type="text/javascript">
	...
	{		
		...
		<hr />		
		<h3>Nhập danh sách tài sản cố định</h3>
		<a <?php if(strlen($ma[0]) >= 16) echo "style='display: none'";?> onclick="showAjaxModal();" class='btn btn-success'>
			Thêm mới
		</a>
		<?php 	
	...
	{
		$sqlm = "select left(mataisanqd32,4) as MA, right(mataisanqd32,4) as SO from tbldanhsachqd32 where mataisanqd32 like 'T%' order by mataisanqd32";
		...
		$phantramhaomon	= $_POST["phantramhaomon"];
		$sql="INSERT INTO tbldanhsachqd32(mataisanqd32,hinhthaitaisan,chitiethinhthai,tentaiqd32,dacdiemtaisan,namsudung,phantramhaomon)
		VALUES ('$macc','$hinhthaitaisan','$chitiethinhthai','$tentaiqd32','$dacdiemtaisan',$namsudung,$phantramhaomon)";				
...
<?php
	if (isset($_POST['chinhsuatss'])) 
	{
		...
		$namsudung = $_POST["namsudungcs"];
		$phantramhaomon	= $_POST["phantramhaomoncs"];
		...
		$sql= $sql."namsudung=$namsudung,phantramhaomon=$phantramhaomon Where mataisanqd32 = '".$_POST['bookId']."'";			
		$kq=mysqli_query($con,$sql);			
	}        
?>
		<table style="line-height: 0;" class="stripe row-border order-column" id="table-2">
			<thead>
				<tr>
					<th>Mã số tài sản</th>
					<th>Hình thái tài sản</th>
					<th>Chi tiết hình thái tài sản</th>
					<th>Tên gọi tài sản</th>
					<th>Đặc điểm tài sản</th>
					<th>Năm sử dụng</th>
					<th>Phần trăm</th>
					<th></th>
				</tr>
			...
			<tbody>	
<?php 
$sql = "select mataisanqd32,hinhthaitaisan,chitiethinhthai,tentaiqd32,dacdiemtaisan,namsudung,phantramhaomon from tbldanhsachqd32 where mataisanqd32 like 'T%'";
$kq = mysqli_query($con,$sql);   
$tatca = ""; 
	...
	$tatca = $row[0].">".$row[1].">".$row[2].">".$row[3].">".$row[4].">".$row[5].">".$row[6];
            echo"<tr>";
				...
				if(strlen($ma[0]) < 16)
				{
				   ...
				   {
					   echo"<td>
						<a title='Chỉ đơn vị tổng hợp mới được thêm sửa, xóa! ' class=' btn btn-warning ' >
							Sửa</a>
						<a  title='Chỉ đơn vị tổng hợp mới được thêm sửa, xóa! ' class='btn btn-warning '>
		...
		<div class="modal-dialog">
			<div class="modal-content">				
				<div class="modal-header">
					...
					<h4 class="modal-title" style="text-align:center;"><strong>Nhập thông tin tài sản cố định</strong></h4>
				</div>				
				<div class="modal-body">				
						<form role='form' action = 'DMTS.php' method='POST' class='form-horizontal form-groups-bordered'>						
							<div class="form-group">
								<label for="field-1" class="col-sm-3 control-label">Mã số tài sản:</label>
								<div class="col-sm-8">
									<input name="mataisanqd32" readonly="false" type="text" class="form-control" id="field-1">
								</div>								
								<label class="col-sm-3 control-label">Hình thái tài sản:</label>																
								<?php								
								$sql="select hinhthaitaisan from tbldanhsachqd32 Where mataisanqd32 like 'T%' Group by hinhthaitaisan";
								...
								<?php								
								$sql="select chitiethinhthai from tbldanhsachqd32 Where mataisanqd32 like 'T%' Group by chitiethinhthai";
								$sql1=mysqli_query($con,$sql);
								echo"<div  class='col-sm-8'>";
								...
								<label class="col-sm-3 control-label">Số năm sử dụng:</label>
								<div class="col-sm-8">
									<input name="namsudung" type="text" class="form-control" id="ttcanhan">
								</div>
								<label class="col-sm-3 control-label">Tỉ lệ hao mòn:</label>
								<div class="col-sm-8">
									<input name="phantramhaomon" type="text" class="form-control" id="ttcanhan">
								</div>								
							</div>
...
<div class="modal fade" id="addBookDialog">
		<div class="modal-dialog">
			<div class="modal-content">				
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
					<h4 class="modal-title" style="text-align:center;"><strong>Chỉnh sửa thông tài sản cố định</strong></h4>
				</div>				
				<div class="modal-body">				
						<form role='form' action = 'DMTS.php' method='POST' class='form-horizontal form-groups-bordered'>
							<div class="form-group">
								...
								<label class="col-sm-3 control-label">Số năm sử dụng:</label>
								<div class="col-sm-8">
									<input name="namsudungcs" type="text" class="form-control" id="namsudungcs">
								</div>
								<label class="col-sm-3 control-label">Tỉ lệ hao mòn:</label>
								<div class="col-sm-8">
