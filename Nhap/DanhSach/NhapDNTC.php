...
$(document).on("click", ".open-AddBookDialog", function (e) {	
	...
	$("#chitiethinhthai").val(myBookId[1]);
	$("#mataisan").val(myBookId[2]);
	$("#tentaisan").val(myBookId[3]);
	$("#tengoi").val(myBookId[4]);	
	$("#caphang").val(myBookId[5]);
	$("#ngaythang").val(myBookId[6]);
	$("#hinhthuc").val(myBookId[7]);
	$("#soluong").val(myBookId[8]);
	$("#sotien").val(myBookId[9]);
	$("#lydo").val(myBookId[10]);
 ...
 <script type="text/javascript">
	...
	function showAjaxModal1()
	{
		...
		window.location.assign('NhapDNTC.php?id='+giatri);			
	}
</script>
<?php
	$tttc=$_GET['id'];	
...
function gettinhhuyen()	
{
	...
	{			
		$up = "UPDATE tbltrangcap SET tinhtrang='Đã duyệt',xemt ='Đã xem',lydo='$_POST[lydo]' WHERE TTcap = ".$_POST['bookId'];		
		$kq = mysqli_query($con,$up);
	}	
	if (isset($_POST['khongduyet']))
	{			
		$up = "UPDATE tbltrangcap SET tinhtrang='Không duyệt',xemt ='Đã xem',lydo='$_POST[lydo]' WHERE TTcap = ".$_POST['bookId'];		
		$kq = mysqli_query($con,$up);			
	}
if($tttc != "0")
{
	if(gettinhhuyen() == "Tỉnh")		
		$sql = "select TTcap,chitiethinhthai,mataisan,tentaisan,tengoi,caphang,ngaythang,hinhthuc,soluong,sotien,tinhtrang,lydo,tendv from tbltrangcap where madonvi like '$ma[0]%'".
...
if(mysqli_num_rows($kq)>0){
	...
	$stt++;
	$xoa =  $row[0].">".$ma[0];
	$tatca = $row[0].">".$row[1].">".$row[2].">".$row[3].">".$row[4].">".$row[5].">".doingay1($row[6]).">".$row[7].">".$row[8].">".$row[9].">".$row[11];
	$matd=$row[0]."/".$tttc."/".gettinhhuyen();
            echo"<tr>";
                echo"<td>$stt</td>";
                echo"<td>$row[10]</td>";				
				echo"<td>$row[11]</td>"; 
				...
				echo"<td>
						<a data-id='$tatca' title='Add this item' class='open-AddBookDialog btn btn-blue ' href='#addBookDialog'>
							Thao tác</a>
						<a href=\"tinhtrangtc.php?id=$matd\" class='btn btn-info '></i>
							Xem</a>												
					</td>";				
            echo"</tr>";
     }}         
		...
		<div class="modal-dialog">
			<div class="modal-content">				
				<div class="modal-header">
					...
					<h4 class="modal-title" style="text-align:center;"><strong>Duyệt đề nghị trang cấp</strong></h4>
				</div>				
				<div class="modal-body">				
						<form role='form' method='POST' action = 'NhapDNTC.php' class='form-horizontal form-groups-bordered'>
							<div class="form-group">								
								<label class="col-sm-3 control-label">TT Trang cấp:</label>
								<div class="col-sm-8">
										<input name="bookId" type="text" class="form-control" id="bookId">									
								</div>								
								<label for="field-1" class="col-sm-3 control-label">Hình thái:</label>
								<div class="col-sm-8">
									<input name="chitiethinhthai" readonly="false" type="text" class="form-control" id="chitiethinhthai">
								</div>
								<label for="field-2" class="col-sm-3 control-label">Mã tài sản:</label>
								<div class="col-sm-8">
									<input name="mataisan" readonly="false" type="text" class="form-control" id="mataisan">
								</div>
								<label for="field-3" class="col-sm-3 control-label">Tên tài sản:</label>
								<div class="col-sm-8">
									<input name="tentaisan" readonly="false" type="text" class="form-control" id="tentaisan">
								</div>
								<label for="field-4" class="col-sm-3 control-label">Tên chi tiết:</label>
								<div class="col-sm-8">
									<input name="tengoi" type="text" readonly class="form-control" id="tengoi">
								</div>
								<label for="field-4" class="col-sm-3 control-label">Cấp hạng:</label>
								<div class="col-sm-8">
									<input name="caphang" type="text" readonly class="form-control" id="caphang">
								</div>
								<label for="field-5" class="col-sm-3 control-label">Ngày tháng:</label>
								<div class="col-sm-8">
									<input data-inputmask="'alias': 'date'" name="ngaythang" readonly type="text" class="form-control" id="ngaythang">
								</div>
								<label for="field-6" class="col-sm-3 control-label">Hình thức:</label>
								<div class="col-sm-8">
									<input name="hinhthuc" type="text" readonly class="form-control" id="hinhthuc">										
								</div>
								<label for="field-6" class="col-sm-3 control-label">Số lượng:</label>
								<div class="col-sm-8">
									<input name="soluong" type="text" readonly class="form-control" id="soluong">
								</div>
								<label for="field-6" class="col-sm-3 control-label">Dự toán:</label>
								<div class="col-sm-8">
									<input name="sotien" type="text" readonly class="form-control" data-mask="fdecimal" id="sotien">
								</div>
								<label for="field-6" class="col-sm-3 control-label">Lý do:</label>
								<div class="col-sm-8">
							...
							<div class="modal-footer">
								...
								<input type="submit" name="duyet" class="btn btn-blue" value = "Duyệt">
								<input type="submit" name="khongduyet" class="btn btn-red" value = "Không duyệt">
							</div>	
						</form>						
				</div>
			</div>
