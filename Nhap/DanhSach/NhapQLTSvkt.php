...
$(document).on("click", ".open-AddBookDialog", function (e) {	
 ...
 <script type="text/javascript">
	...
	{
		//giatri = $('#dientich').val()+">"+$('#noisudung').val()+">"+$('#namsd').val()+">"+$('#namnhap').val()+">"+$('#xem').val();
		giatri = $('#dientich').val()+">"+$('#noisudung').val()+">"+$('#namsd').val()+">"+$('#namnhap').val()+">"+$('#xem').val();
		window.location.assign('NhapQLTSvkt.php?id='+giatri);			
	}
</script>				
		<?php
	$tttc=$_GET['id'];	
...
?>		
	...
	<div class="panel-body">
		...
		<label class='col-sm-15' style = "margin-top:7px;" >Diện tích:</label>
		<div class="col-sm-3">
			<input name="dientich"  type="text" placeholder="Diện tích xây dựng"  class="form-control" id="dientich" <?php if($key[0] != "0") echo "value='$key[0]'"; ?>>
		</div>	
...
if($tttc != "0")
{
	if(gettinhhuyen() == "Tỉnh")
		...
		"noidung,DVT,GCTS,ttkhac from tblqlts where madonvi like '$ma[0]%' and tblqlts.chitiethinhthai = 'Vật kiến trúc' and chitiethinhthai <> 'Nhà' and".
		" chitiethinhthai not Like 'Phương tiện vận tải%' and chitiethinhthai not like 'Đất%' and chitiethinhthai not like 'Quyền sử dụng đất%'".
		($key[1]==""?"":" and madonvi='".$key[1]."'").($key[0]==""?"":" and DTXD='".$key[0]."'").($key[2]==""?"":" and Year(ngaysudung)='".$key[2]."'").($key[3]==""?"":" and Year(ngaynhap)='".$key[3]."'").($key[4]==""?"":" and xemt ='".$key[4]."'");
	else
		$sql = "select TTQLTS,mataisan,tentaisan,tenchitiet,ngaynhap,namsanxuat,NUOCSX,ngaysudung,DTKV,DTXD,ngansach,nguonkhac,chitiethinhthai,".
		"noidung,DVT,GCTS,ttkhac from tblqlts where madonvi like '$ma[0]%' and tblqlts.chitiethinhthai = 'Vật kiến trúc' and chitiethinhthai <> 'Nhà' and".
		" chitiethinhthai not Like 'Phương tiện vận tải%' and chitiethinhthai not like 'Đất%' and chitiethinhthai not like 'Quyền sử dụng đất%'".
		($key[1]==""?"":" and madonvi='".$key[1]."'").($key[0]==""?"":" and DTXD='".$key[0]."'").($key[2]==""?"":" and Year(ngaysudung)='".$key[2]."'").($key[3]==""?"":" and Year(ngaynhap)='".$key[3]."'").($key[4]==""?"":" and xemh ='".$key[4]."'");
}	
else
	$sql = "select TTQLTS,mataisan,tentaisan,tenchitiet,ngaynhap,namsanxuat,NUOCSX,ngaysudung,DTKV,DTXD,ngansach,nguonkhac,chitiethinhthai,".
	"noidung,DVT,GCTS,ttkhac from tblqlts where madonvi like '$ma[0]%' and tblqlts.chitiethinhthai = 'Vật kiến trúc' and chitiethinhthai <> 'Nhà' and".
...
if(mysqli_num_rows($kq)>0){
	while($row=mysqli_fetch_array($kq)){	
			...
			$matd=$row[0]."/".$tttc."/vkt/".gettinhhuyen();
            echo"<tr>";
                echo"<td>$stt</td>";
                echo"<td>$row[2]</td>";
                echo"<td>$row[3]</td>";
                ...
                echo"<td>
						<a href=\"tinhtrangts.php?id=$matd\" class='btn btn-info '></i>
							Xem</a>	
						<a href=\"NhapQLTSvktsua.php?id=$row[0]\" class='btn btn-blue '></i>
							Sửa</a>	
					</td>";
            echo"</tr>";
    }}         
