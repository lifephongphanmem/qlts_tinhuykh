...
$(document).on("click", ".open-AddBookDialog", function (e) {	
 ...
 <script type="text/javascript">
	...
	{
		giatri = $('#phanlsd').val()+">"+$('#bienso').val()+">"+$('#noisudung').val()+">"+$('#namsd').val()+">"+$('#namnhap').val()+">"+$('#xem').val();
		window.location.assign('NhapQLTSvtk.php?id='+giatri);			
	}
</script>	
<?php
	$tttc=$_GET['id'];	
...
?>		
	...
	<div class="panel panel-dark">
		<div class="panel-heading">
			<div class="panel-title">
				Quản lý xe và phương tiện vận tải
			</div>	
	...
	<div class="panel-body">
		...
		<label class='col-sm-15' style = "margin-top:7px;">Phân loại sử dụng:</label>
		<div  class='col-sm-4' >
			<select name='phanlsd' class='form-control' id='phanlsd'>					
				<?php
					...
					{
						if($key[0] == "")
							echo "<option selected='selected' value=''>Chọn loại sử dụng</option>";
						else
						{
							echo "<option value =''> Chọn loại sử dụng</option>";
						...
						echo "<option selected='selected' value=''>Chọn loại sử dụng</option>";
				?>
				<option  value='Xe phục vụ chức danh'>Xe phục vụ chức danh</option>
				<option  value='Xe phục vụ chung'>Xe phục vụ chung</option>
				<option  value='Xe chuyên dùng'>Xe chuyên dùng</option>
			</select>
		</div>
		<label class='col-sm-15' style = "margin-top:7px;" >Biển số:</label>
		<div class="col-sm-4">
			<input name="bienso" type="text" placeholder="Biển số"  class="form-control" id="bienso" <?php if($key[0] != "0") echo "value='$key[1]'"; ?>>
		</div>	
		...
		<a  onclick="showAjaxModal1();" class='btn btn-success' style = "margin-top:32px;">
			Tìm kiếm
		</a>		
	</div>
	</div>	
...
if($tttc != "0")
{
	if(gettinhhuyen() == "Tỉnh")
		$sql = "select TTQLTS,mataisan,tentaisan,tenchitiet,ngaynhap,ngaysudung,noidung,namsanxuat,NUOCSX,BKSXE,NHXE,CNXE,CXXE,NGGOXE,PLSD,CDSD,ngansach,nguonkhac,ttkhac from tblqlts where madonvi like '$ma[0]%' and chitiethinhthai not Like 'Phương tiện vận tải đường bộ%' and chitiethinhthai Like 'Phương tiện vận tải%'".
		($key[0] ==""?"":" and PLSD ='".$key[0]."'").($key[2]==""?"":" and madonvi='".$key[2]."'").($key[1]==""?"":" and BKSXE='".$key[1]."'").($key[3]==""?"":" and Year(ngaysudung)='".$key[3]."'").($key[4]==""?"":" and Year(ngaynhap)='".$key[4]."'").($key[5]==""?"":" and xemt ='".$key[5]."'");	
	else
		$sql = "select TTQLTS,mataisan,tentaisan,tenchitiet,ngaynhap,ngaysudung,noidung,namsanxuat,NUOCSX,BKSXE,NHXE,CNXE,CXXE,NGGOXE,PLSD,CDSD,ngansach,nguonkhac,ttkhac from tblqlts where madonvi like '$ma[0]%' and chitiethinhthai not Like 'Phương tiện vận tải đường bộ%' and chitiethinhthai Like 'Phương tiện vận tải%'".
		($key[0] ==""?"":" and PLSD ='".$key[0]."'").($key[2]==""?"":" and madonvi='".$key[2]."'").($key[1]==""?"":" and BKSXE='".$key[1]."'").($key[3]==""?"":" and Year(ngaysudung)='".$key[3]."'").($key[4]==""?"":" and Year(ngaynhap)='".$key[4]."'").($key[5]==""?"":" and xemh ='".$key[5]."'");	
}
else
	$sql = "select TTQLTS,mataisan,tentaisan,tenchitiet,ngaynhap,ngaysudung,noidung,namsanxuat,NUOCSX,BKSXE,NHXE,CNXE,CXXE,NGGOXE,PLSD,CDSD,ngansach,nguonkhac,ttkhac from tblqlts where madonvi like '$ma[0]%' and chitiethinhthai not Like 'Phương tiện vận tải đường bộ%' and chitiethinhthai Like 'Phương tiện vận tải%'";			
	...
	if(mysqli_num_rows($kq)>0){
		while($row=mysqli_fetch_array($kq)){		
			$stt++;
				$tatca = $row[0].">".$row[1].">".$row[2].">".$row[3].">".doingay1($row[4]).">".doingay1($row[5]).">".$row[6].">".$row[7].">".$row[8].">".$row[9].">".$row[10].">".$row[11].">".$row[12].">".$row[13].">".$row[14].">".$row[15].">".$row[16].">".$row[17].">".$row[18];			
				$matd=$row[0]."/".$tttc."/ptvt";
				echo"<tr>";
					echo"<td>$stt</td>";
					echo"<td>$row[2]</td>";
					echo"<td>$row[3]</td>";
					...
					echo"<td>
							...
							Xem</a>	
						<a href=\"NhapQLTSxesua.php?id=$row[0]\" class='btn btn-blue '></i>
							Sửa</a>	
						</td>";
				echo"</tr>";
		 }}  
