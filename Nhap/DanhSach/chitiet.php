...
function conlai($mats)
{
	...
	$sql = "Select sotien,tanggiam from tbltanggiam where TTQLTS = ".$mats;
	$qrsql = mysqli_query($con,$sql);
	while ($row = mysqli_fetch_array($qrsql))
	{
		if($row['tanggiam'] == "Tăng")
			$tanggiam+= doubleval($row['sotien']);
		if($row['tanggiam'] == "Giảm")
			$tanggiam -= doubleval($row['sotien']);
	}	
	$kq=$nguyengia-$haomon+$tanggiam;
	return $kq;
}
$sql = "Select tentaisan,tenchitiet,ngaynhap,ngaysudung,namsanxuat,NUOCSX,noidung,DVT,DTKV,ngansach,nguonkhac,GCTS,ttkhac from tblqlts where TTQLTS = '$key[0]'";
					...
					<div class="panel-body">
						<form role="form" method="POST" class="form-horizontal form-groups-bordered">
							<div class="form-group">
								...
								<div class="col-md-4">
									<input name="ngansach" class="form-control" type="text" value = "<?php $ngngansach=dinhdangso($row[9]); echo $ngngansach; ?>">
								</div>
								<label  class="col-sm-2 control-label">Nguồn khác:</label>
								<div class="col-md-4">
									<input name="nguonkhac" class="form-control" type="text" value = "<?php echo dinhdangso($row[10]); ?>">
								</div>	
								...
								<label for="field-4" class="col-sm-2 control-label">Thông số kt:</label>
								<div class="col-sm-4">
									<input name="GCTS" type="text" class="form-control" id="field-3" value = "<?php echo $row[11]; ?>">
								</div>
								<label  class="col-sm-2 control-label">Ghi chú:</label>
								<div class="col-md-4">
									<input name="ttkhac" class="form-control" type="text" value = "<?php echo $row[12]; ?>">
								</div>
							</div>
							<div>
...
?>									
							...
							<div class="in">
								<input type="button" class="btn btn-black" value="Trở lại" onclick="window.location.href='NhapQLTS.php?id=<?php echo "$key[1]"; ?>'">
