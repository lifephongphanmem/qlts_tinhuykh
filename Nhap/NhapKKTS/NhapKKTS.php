...
<link rel="stylesheet" type="text/css" href="/assets/News/dataTables.fixedColumns.css">
	...
	<script type="text/javascript">
		function onclick() {
				return "/Tienich/KkTSCDexxcel.php";
		}
	</script>
		<hr />
		<h3>Kiểm kê tài sản cố định</h3>
<?php		
		echo "<a href='themkk.php' class='btn btn-success'>
			Thêm mới kiểm kê tài sản
		</a>";
		echo "<a href='themkkcc.php' class='btn btn-blue'>
			Thêm mới kiểm kê công cụ
		</a>";
		//echo "<a href='themktra.php' class='btn btn-red'>
		//	Thêm mới kiểm tra hiện trạng
		//</a>";
?>		
...
$ab = $ma[0];
$sql = "select TTkiemke,GIO,PHUT,ngay1,sohieu,noikiemke,noidung,ttkhac from tblkiemke where madonvi = '$ab'";
$kq = mysqli_query($con,$sql);
$matm = "";  $stt=0; 
if(mysqli_num_rows($kq)>0){
	while($row=mysqli_fetch_array($kq)){
	$matm = $row[0];
	//$main = $madv[0].">".$madv[1].">".$madv[2].">"." ".">".$madv[4].">".$madv[5].">".$madv[6].">".$madv[7].">".$madv[8].">".$madv[9].">".$madv[10].">".$madv[11].">".$madv[12].">".$row[0];
	$main = $ma[0].">".$row[0];
		$stt++;
            echo"<tr>";
                echo"<td>$stt</td>";
                echo"<td>$row[1].$row[2]</td>";
                echo"<td>".doingay1($row[3])."</td>";
                echo"<td>$row[4]</td>";
                echo"<td>$row[5]</td>";
				echo"<td>$row[6]</td>";
				echo"<td>$row[7]</td>";								
                echo"<td>
						<a href=\"suakk.php?id=$matm\" class='btn btn-blue '>
							Sửa
						</a>";
					if($row['ttkhac'] == "Kiểm kê công cụ")
						echo "<a href=\"NhapKKTSccct.php?id=$matm\" class='btn btn-yellow '>
							Sửa chi tiết
						</a>";
					else
						echo "<a href=\"NhapKKTSct.php?id=$matm\" class='btn btn-yellow '>
							Sửa chi tiết
						</a>";
						echo "<a href=\"xoa.php?id=$matm\" onclick=\"return confirm('Bạn muốn xóa kiểm kê tài sản này?')\" class='btn btn-red '>
							Xóa
						</a>";
					if($row['ttkhac'] == "Kiểm kê công cụ")
						echo "<a href=\"BbKKCCDC107.php?id=$main\" class='btn btn-green '>
							In
						</a>";
					else {
						echo "<a href=\"BbKKTSCD.php?id=$main\" class='btn btn-green '>
							In
						</a>";
						echo "<a href='\..\Tienich\KkTSCDexcel.php?id=$main' class='btn btn-yellow '>
							Excel
						</a>";
					}
					echo  "</td>";
