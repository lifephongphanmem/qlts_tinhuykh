    ...
    function doingay1($ngay)
{
	...
	$ttkk = 0;
	$ab = $ma[0];
	$GIO="";$PHUT ="";$ngay1="";$sohieu ="";$noikiemke ="";$noidung = "";$ttkhac = "";
	...
	{			
		...
		function checkts($idts)
		{
			global $ngay1;
			global $con;
			$sql ="select TTQLTS from tbldenghi where (hinhthuc  = 'Thanh lý' or hinhthuc  = 'Điều chuyển' or hinhthuc  = 'Thu hồi' or hinhthuc  = 'Bán')".
				" and ngaythang <='".$ngay1."' and TTQLTS =".$idts;
		...
		$sql="INSERT INTO tblkiemke(GIO,PHUT,ngay1,sohieu,noikiemke,noidung,ttkhac,madonvi,xemt,xemh) VALUES ('$GIO','$PHUT','$ngay1','$sohieu','$noikiemke','$noidung','Kiểm kê tài sản','$ab','Chưa xem','Chưa xem')";
		$kq=mysqli_query($con,$sql);
		$sql1 = "Select TTkiemke From tblkiemke Where madonvi = '$ab' order by TTkiemke";		
		$kq1=mysqli_query($con,$sql1);
		while($row1=mysqli_fetch_array($kq1)){
			$ttkk = $row1[0];
		}		
		//$sql = "Update  tblthanhphankk Set TTkiemke = ".$ttkk." where madonvi = '$ab' and (TTkiemke is null or TTkiemke = 0)";
		//$kq=mysqli_query($con,$sql);	
		$sql = "Select * From tblqlts where ngaysudung <= '".$ngay1."' and madonvi = '$ab'".
			" and TTQLTS not in (select TTQLTS from tbldenghi where (hinhthuc  = 'Thanh lý' or hinhthuc  = 'Điều chuyển' or hinhthuc  = 'Thu hồi' or hinhthuc  = 'Bán')".
			" and ngaythang <='".$ngay1."')";
		$kq=mysqli_query($con,$sql);
		$tentscc="";$tenchitiet="";$nhom1sllt=0;$nhom1nglt=0;$nhom1cllt=0;$chitiethinhthai="";$noisudung="";$dvt="";$TTQLTS ="";
		while($row=mysqli_fetch_array($kq)) {
			$tanggiam = 0;
			$sqltg = "Select sotien,tanggiam From tbltanggiam where TTQLTS =".$row['TTQLTS'];
			$qrsqltg = mysqli_query($con,$sqltg);
			while ($rowtg = mysqli_fetch_array($qrsqltg)){
				if($rowtg['tanggiam'] == "Tăng")
					$tanggiam += $rowtg['sotien'];
				if($rowtg['tanggiam'] == "Giảm")
					$tanggiam -= $rowtg['sotien'];
			}
			$luykehm = 0;
			$sqlhm = "Select sodu,sotien from tblhaomon where TTQLTS = " . $row['TTQLTS'];
			$kqhm = mysqli_query($con, $sqlhm);
			while ($rowhm = mysqli_fetch_array($kqhm)) {
				$luykehm = $rowhm['sodu'] + $rowhm['sotien'];
			}
			$tentscc = $row['tentaisan'];
			$tenchitiet = $row['tenchitiet'];
			$nhom1sllt = ($row['DTKV'] + $row['DTXD']);
			if(checkts($row['TTQLTS']))
			$nhom1nglt = 0;
			else
			$nhom1nglt = ($row['ngansach'] + $tanggiam + $row['nguonkhac']);
			if(checkts($row['TTQLTS']))
			$nhom1cllt = 0;
			else
			$nhom1cllt = ($row['ngansach'] + $tanggiam +  $row['nguonkhac']) - $luykehm;
			$chitiethinhthai = $row['chitiethinhthai'];
			$noisudung = $row['noidung'];
			$dvt = $row['DVT'];
			$TTQLTS = $row['TTQLTS'];
			$sqlkkct = "INSERT INTO tblkiemken(TTkiemke,tentscc,tenchitiet,nhom1sllt,nhom1nglt,nhom1cllt,nhom2sltt,nhom2ngtt,nhom2cltt,chitiethinhthai,noisudung,dvt,luykehaomon,madonvi,TTQLTS )";
			$sqlkkct = $sqlkkct . " values ($ttkk,'$tentscc','$tenchitiet',$nhom1sllt,$nhom1nglt,$nhom1cllt,$nhom1sllt,$nhom1nglt,$nhom1cllt,'$chitiethinhthai','$noisudung','$dvt',$luykehm,'$ab','$TTQLTS ')";
			$kqkkct = mysqli_query($con, $sqlkkct);
		}
					...
					<div class="panel-body">
						<form role="form" method="POST" class="form-horizontal form-groups-bordered"> 
							<div class="form-group">
								<label  class="col-sm-15 control-label">Giờ KK:</label>
								<div class="col-sm-1">
									<input name="GIO" class="form-control" type="text" value = "<?php echo $GIO ?>">
								</div>
								<label  class="col-sm-15 control-label">Phút KK:</label>
								<div class="col-sm-1">
									<input name="PHUT" class="form-control" type="text" value = "<?php echo $PHUT ?>">
								</div>
								<label  class="col-sm-15 control-label">Ngày nhập BB:</label>
								<div  class="col-sm-3">
									<input data-inputmask="'alias': 'date'" name="ngay1" type="text" class="form-control" id="field-1" value = "<?php echo doingay1($ngay1) ?>">
								</div>
							</div>
							<div class="form-group">
								<label  class="col-sm-15 control-label">Số hiệu BB:</label>
								<div class="col-sm-1">
									<input name="sohieu" type="text" class="form-control" value = "<?php echo $sohieu ?>">
								</div>
								<label  class="col-sm-15 control-label">Nơi KK:</label>
								<div  class="col-sm-55">
									<input name="noikiemke" type="text" class="form-control" value = "<?php echo $noikiemke ?>">
								</div>
							</div>
							<div class="form-group">
								<label  class="col-sm-15 control-label">Nội dung:</label>
								<div class="col-sm-8">
									<input name="noidung" type="text" class="form-control" value = "<?php echo $noidung ?>">
								</div>
							</div>
							<div class="form-group">
								<label  class="col-sm-15 control-label">Ghi chú:</label>
								<div class="col-sm-8">
									<input name="ttkhac" readonly type="text" class="form-control" value = "<?php echo $ttkhac ?>">
								</div>								
							</div>
							<div class="form-group">
								<input type="submit" id="save" name="capnhatkk" <?php if($ttkk != 0) echo "style = 'display: none';"?> class="btn btn-info" value = "Cập nhật">
								<input type="button" id="addkk" <?php if($ttkk == 0) echo "style = 'display: none';"?> class="btn btn-info" value="Thêm thành phần ban kiểm kê" onclick="<?php echo "window.location.href='themtp.php?id=$ttkk'" ?>" />
