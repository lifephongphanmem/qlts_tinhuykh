...
<script type="text/javascript">
	...
	{		
		...
		<hr />			
		<h3>Nhập thành phần ban kiểm tra</h3>
...
<?php 	
	...
	{	
		...
		$sql="INSERT INTO tblthanhphankk(TTkiemke,hotencb,chucvucanbo,daidientp,madonvi) VALUES ('$a','$hoten','$chucvu','$daidien','$ab')";
		$kq=mysqli_query($con,$sql);		
	}	        	
?>		
<?php
	if (isset($_POST['chinhsuatss'])) 
	{
		...
		$sql="Update tblthanhphankk set hotencb='$hoten',chucvucanbo='$chucvu',daidientp='$daidien' where TTthanhphan=".$_POST['bookId'];
		$kq=mysqli_query($con,$sql);				
	}        
?>		
...
$sql = "select TTthanhphan,hotencb,chucvucanbo,daidientp,thanhphan,ttten from tblthanhphankk Where madonvi = '$ab' and TTkiemke = '$a'";
	$kq = mysqli_query($con,$sql);
	$ID = 0;$tatca = "";   $stt=0;
		...
		$maxoa = $row[0].">".$a;	$stt++;	
				echo"<tr>";
					...
					echo"<td>												
							...
							<a href=\"xoatpktra.php?id=$maxoa\" onclick=\"return confirm('Bạn muốn xóa thành phần kiểm tra này?')\" class='btn btn-danger btn-sm '>
								Xóa
							</a>						
						</td>";
				echo"</tr>";
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-header">
					...
					<h4 class="modal-title" style="text-align:center;"><strong>Thêm mới thành phần ban kiểm tra hiện trạng</strong></h4>
				</div>				
				<div class="modal-body">				
						<?php echo "<form role='form' method='POST' action ='themtpktra.php?id=$a' class='form-horizontal form-groups-bordered'>";	?>
...
<div class="modal fade" id="addBookDialog" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
        <h4 class="modal-title" id="myModalLabel">Chỉnh sửa thành phần ban kiểm tra hiện trạng</h4>
      </div>
      <div class="modal-body">        
		<form role='form' method='POST' action =<?php echo "'themtp.php?id=$a'";?> class='form-horizontal form-groups-bordered'>
							<div class="form-group">
							...
							<div class="form-group">
								<label class="col-sm-3 control-label">Họ tên cán bộ:</label>
								<div class="col-sm-5">
									<input name="hoten1"  type="text" class="form-control" id="hoten1" value = "">
								</div>
								<?php
								/*
								$sql="select STTcanbo,hoten from tbldanhsachcb where madonvi = '$ab'";
							...
							<div class="form-group">
								<label for="field-3" class="col-sm-3 control-label">Chức vụ cán bộ:</label>
								<div class="col-sm-5">
									<input name="chucvucanbo"  type="text" class="form-control" id="chucvucanbo1" value = "">
								</div>
							</div>
							<div class="form-group">
								<label for="field-4" class="col-sm-3 control-label">Đại diện:</label>
