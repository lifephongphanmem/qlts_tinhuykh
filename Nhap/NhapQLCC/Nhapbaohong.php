...
$(document).on("click", ".open-AddBookDialog", function (e) {
	...
	$("#noibaohong").val(myBookId[6]);
	$("#soluong").val(myBookId[8]);
	$("#dongia").val(myBookId[9]);
	$("#sotien").val(myBookId[10]);
	$("#xx").val(myBookId[1]+">"+myBookId[2]+">"+myBookId[3]+">"+myBookId[4]+">"+myBookId[5]+">"+myBookId[7]+">"+myBookId[11]+">"+myBookId[12]);
    $(_self.attr('href')).modal('show');	
});
 </script>
		<hr />	
		<h3>Quản lý báo hỏng công cụ dụng cụ</h3>
		<input type="button" class="btn btn-black" value="Trở lại" onclick="<?php echo "window.location.href='NhapQLCC.php'" ?>" />	
<?php
	$ab = $ma[0];
	$id = $_GET['id'];
	function sohong ($ttxuatdung,$phanloai)
	{
		$slhong =0;
		$slmat = 0;
		$slkhac = 0;
	    $kq =0;
		global $con;
	    $sql = "Select 	soluong,phanloai from tblbaohong where ttxuatdung = $ttxuatdung";
	    $qrsql = mysqli_query($con,$sql);
	    while ($row = mysqli_fetch_array($qrsql))
	    {
			if($row['phanloai'] == "Hỏng" )
	        	$slhong += kieudouble($row['soluong']);
			if($row['phanloai'] == "Mất" )
				$slmat += kieudouble($row['soluong']);
			if($row['phanloai'] != "Hỏng" && $row['phanloai'] != "Mất" )
				$slkhac += kieudouble($row['soluong']);
	    }
		if($phanloai == "Hỏng")
			$kq = $slhong;
		if($phanloai == "Mất")
...
function ktraslhong ($ttxuatdung)
{
	$slhong =0;
	$slmat = 0;
	$slkhac = 0;
	$kq =0;
	global $con;
	$sql = "Select 	soluong from tblbaohong where ttxuatdung = $ttxuatdung";
	$qrsql = mysqli_query($con,$sql);
	while ($row = mysqli_fetch_array($qrsql))
	{
	...
	function ktra($ttxuatdung)
	{
	    global $con;
	    $kq=0;
	    $sohong = ktraslhong($ttxuatdung);
	    $sql = "Select soluong from tblxuatdung where ttxuatdung = $ttxuatdung";
	    $qrsql = mysqli_query($con,$sql);
	    while ($row = mysqli_fetch_array($qrsql))
	    {
	        $kq += kieudouble($row['soluong']);
	...
	{		
		...
		$th = explode(">",$_POST["xx"]);
		$sql = "Insert into tblbaohong(ttxuatdung,TTQLcc,ngaythang,phanloai,Lydohong,soluong,sotien,mataisan,tentaisan,tenchitiet,nuocsanxuat,namsx,dvt,chitiethinhthai,dongia,noibaohong,madonvi)";
		$sql = $sql." values('$th[7]',$TTQLcc,'$ngaythang','$phanloai','$Lydohong',$soluong,$sotien,'$th[0]','$th[1]','$th[2]','$th[4]','$th[3]','$th[5]','$th[6]',$dongia,'$noibaohong','$ab')";
		...
		<table style="line-height: 0;" class="stripe row-border order-column" id="table-2">
			<thead>
				<tr>
					...
					<th>Tên công cụ</th>
					<th>Tên chi tiết</th>
					<th width="1%">Ngày sử dụng</th>
					<th>Nơi sử dụng</th>
					<th width="1%">Số SD</th>
					<th width="1%">Số hòng</th>
					<th width="1%">Số mất</th>
					<th width="1%">Khác</th>
					<th></th>
				</tr>
			</thead>			
...
//require("connect.php");
$sql = "select TTQLcc,mataisan,tentaisan,tenchitiet,namsanxuat,nuocsx,ngayxuat,noisudung,dvt,soluong,dongia,sotien,chitiethinhthai,ttxuatdung from tblxuatdung where madonvi = '$ab' and TTQLcc = $id";
$kq = mysqli_query($con,$sql);
$ngnh="";$ngsd="";$tatca = "";$matm = "";$stt=0;
if(mysqli_num_rows($kq)>0){
	while($row=mysqli_fetch_array($kq)){	
	$stt++;
	$tatca = $row[0].">".$row[1].">".$row[2].">".$row[3].">".$row[4].">".$row[5].">".$row[7].">".$row[8].">".$row[9].">".$row[10].">".$row[11].">".$row[12].">".$row[13];
	$matm = $row[0].">".$row['ttxuatdung'];
            echo"<tr>";
                echo"<td>$stt</td>";
                echo"<td>$row[2]</td>";
                echo"<td>$row[3]</td>";
				$ngsd = doingay1($row['ngayxuat']);
				echo"<td>$ngsd</td>";	
				echo"<td>$row[7]</td>";
				echo "<td>$row[soluong]</td>";
				echo "<td>".sohong($row['ttxuatdung'],"Hỏng")."</td>";
				echo "<td>".sohong($row['ttxuatdung'],"Mất")."</td>";
				echo "<td>".sohong($row['ttxuatdung'],"Khác")."</td>";
                echo"<td>";
                    if(ktra($row['ttxuatdung']) > 0)
						echo "<a data-id='$tatca' title='Add this item' class='open-AddBookDialog btn btn-primary' href='#addBookDialog'>Báo hỏng</a>";
					else
					    echo "<a title='Đã báo hỏng hết' class=' btn btn-warning'>Báo hỏng</a>";
					echo "<a href=\"Nhapsuabaohong.php?id=$matm\" class='btn btn-info '>
							Chi tiết
						</a>	
					</td>";
            echo"</tr>";
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel">Thêm mới báo hỏng công cụ</h4>
      </div>
      <div class="modal-body">      
		<form role='form' method='POST' action = 'Nhapbaohong.php?id=<?php echo $id;?>' class='form-horizontal form-groups-bordered'>
