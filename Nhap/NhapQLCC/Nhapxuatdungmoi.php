...
$(document).on("click", ".open-AddBookDialog", function (e) {
	...
	$("#soluong").val(myBookId[4]);
	$("#dongia").val(myBookId[5]);
	$("#sotien").val(myBookId[6]);
	$("#xx").val(myBookId[7]+">"+myBookId[8]+">"+myBookId[9]+">"+myBookId[10]);
	$(_self.attr('href')).modal('show');
});
 </script> 
 <?php
...
<?php
	$sql = "select TTQLcc,mataisan,tentaisan,tenchitiet,dongia,chitiethinhthai,namsanxuat,DVT,nuocsx from tblqlcc where TTQLcc = '$id'";
	$kq = mysqli_query($con,$sql);	
	while($row=mysqli_fetch_array($kq)){
		$idmoi = $row['TTQLcc'].">".$row['mataisan'].">".$row['tentaisan'].">".$row['tenchitiet'].">".slcon($row['TTQLcc']).">".$row['dongia'].">".kieudouble(slcon($row['TTQLcc']))*kieudouble($row['dongia']).">".$row['chitiethinhthai'].">".$row['namsanxuat'].">".$row['DVT'].">".$row['nuocsx'];
		//$idmoi = $id.">".slcon($row[0]).">".$row[1].">".kieudouble(slcon($row[0]))*kieudouble($row[1]).">".$row[2].">".$row[3].">".$row[4].">".$row[5];
	}	
	if(slcon($id) > 0)
		echo "<a data-id='$idmoi' title='Add this item' class='open-AddBookDialog btn btn-success' href='#addBookDialog'>Thêm mới</a>	";
?>		
...
$sql = "select ttxuatdung,TTQLcc,mataisan,tentaisan,tenchitiet,ngayxuat,noisudung,soluong,sotien,chitiethinhthai,namsanxuat,dvt,nuocsx,dongia,lydoxuat from tblxuatdung where madonvi = '$ab' and TTQLcc = $id";
$kq = mysqli_query($con,$sql);
$ngsd="";$tatca=""; $stt=0;$maxoa="";
		...
		$maxoa = $row[0].">".$id;
            echo"<tr>";
                ...
                echo"<td>												
						...
						<a href=\"xoaxd.php?id=$maxoa\" onclick=\"return confirm('Bạn muốn xóa tài sản này?')\" class='btn btn-red '>
							Xóa
						</a>						
					</td>";
            echo"</tr>";
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">        
		<form role='form' method='POST' onsubmit="return ktmoi()" action = 'Nhapxuatdungmoi.php?id=<?php echo $id;?>' class='form-horizontal form-groups-bordered'>
							<div class="form-group">
								<input name="bookId" type="text" style="display: none" class="form-control" id="bookId">
 ...
 <div class="modal fade" id="addBookDialogg" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">        
		<form role='form' method='POST' onsubmit="return ktsua()" action = 'Nhapxuatdungmoi.php?id=<?php echo $id;?>' class='form-horizontal form-groups-bordered'>
							<div class="form-group">
