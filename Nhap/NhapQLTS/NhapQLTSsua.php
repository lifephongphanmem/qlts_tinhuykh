...
if(isset($_POST["luupb"])) {
	...
	if (isset($_POST['capnhatts'])) {
		...
		$namkhauhao = kieudouble($_POST['namkhauhao']);
		$tongnamsudung = kieudouble($_POST['tongnamsudung']);
		...
		$sql = "Update tblqlts set mataisan='$mataisan',tentaisan='$tentaisan',tenchitiet='$tenchitiet',ngaynhap='$ngaynhap',namsanxuat='$namsanxuat'" .
			",ngaysudung='$ngaysudung',noidung='$noidung',ngansach=$ngansach,nguonkhac=$nguonkhac,DTKV='$DTKV',NUOCSX='$NUOCSX',chitiethinhthai='$chitiethinhthai'" .
			",XOA='K',GCTS='$GCTS',DVT='$DVT',ttkhac='$ttkhac',phantram=$pthm,sonamsd=$sonamsd,sothutu=$ttsx,madonvi='$ab',phantram32='$phantram32',sohaomon=$sohaomon" .
			",conlai=$conlai,theodoi='$theodoi',quyetdinhbangiao='$quyetdinhbangiao',ngayquyetdinhbangiao='$ngayquetdinhbangiao',hoadon='$hoadon',ngayhoadon='$ngayhoadon'" .
			",lydotang = '$lydotang',giatrikhcl=$giatrikhcl,khauhao='$khauhao',namsudungcl = '$namsudungcl',ngaythangchuyen = '$ngaythangchuyen',nguoisudung = '$nguoisudung'" .
			" WHERE TTQLTS = $id";
		$kq = mysqli_query($con, $sql);
		$sql = "update tblhaomon set ngaythang='$ngaynhap',namhaomon=$namhm,phantram=$phantramhm,sodu=$soduhm,sotien=$sohmnam,ngaykhauhao='$ngaykhauhao',namkhconlai='$namkhconlai'" .
			",phuongphapkhauhao='$phuongphapkhauhao',thangkhauhaoconlai=$thangkhauhaoconlai,tylekhauhao=$tylekhauhao,tylesxkd=$tylesxkd,sokhauhao=$sokhauhao,sokhnam=$sokhnam" .
			",namkhauhao=$namkhauhao,tongnamsudung=$tongnamsudung WHERE TTTS = $id";
		$kq = mysqli_query($con, $sql);
...
$sql = "select tblqlts.mataisan,tblqlts.sonamsd,tblqlts.tenchitiet,tblqlts.ngaynhap,tblqlts.ngaysudung,tblqlts.namsanxuat".
	",tblqlts.NUOCSX,tblqlts.noidung,tblqlts.DVT,tblqlts.DTKV,tblqlts.ngansach,tblqlts.nguonkhac,tblqlts.phantram".
	",tblqlts.phantram32,tblqlts.sohaomon,tblqlts.conlai,tblqlts.GCTS,tblqlts.ttkhac,tblhaomon.namhaomon,tblhaomon.sodu".
	",tblhaomon.sotien,tblhaomon.phantram,tblhientrang.phanloai,tblqlts.quyetdinhbangiao,tblqlts.ngayquyetdinhbangiao".
	",tblqlts.hoadon,tblqlts.ngayhoadon,tblqlts.lydotang,tblqlts.khauhao,tblqlts.giatrikhcl,tblhaomon.ngaykhauhao".
	",tblhaomon.namkhconlai,tblhaomon.phuongphapkhauhao,tblhaomon.thangkhauhaoconlai,tblhaomon.tylekhauhao,tblhaomon.tylesxkd".
	",tblhaomon.sokhauhao,tblhaomon.sokhnam,tblhaomon.namkhauhao,tblhaomon.tongnamsudung,tblqlts.namsudungcl,tblqlts.ngaythangchuyen,tblqlts.nguoisudung".
	" FROM (tblqlts INNER JOIN tblhaomon ON tblqlts.TTQLTS = tblhaomon.TTTS) INNER JOIN tblhientrang ON tblqlts.TTQLTS = tblhientrang.TTTS".
	" WHERE tblqlts.TTQLTS =". $id;
$qrsql = mysqli_query($con,$sql);
while($row = mysqli_fetch_array($qrsql))
{
?>
<div class="col-sm-12">		
  ...
  <legend>Thông tin tài sản</legend>
            <input name="TTQLTS" type="text" style ="display:none" class="form-control" value="<?php echo $id; ?>" id="TTQLTS"/>
			...
			<div class="col-sm-45">
				<input data-inputmask="'alias': 'date'" name="ngaynhap" type="text" tabindex="-1" data-validate = "required" class="form-control" value="<?php echo doingay1($row[3]); ?>" id="ngaynhap"/>
			</div>
			...
			<div class="col-sm-45">									
				<input data-mask="decimal"  name='namsanxuat' id='namsanxuat' tabindex="-1" value="<?php echo $row[5]; ?>" data-validate = "required" class='form-control' />
			</div>
...
<a onclick="showAjaxModal();" class='col-sm-05 btn'><img src="/icon/add.png"  style="padding-left: 0px;" height="16" width="22"/></a>
			...
			<div class="col-sm-45">
				<input data-inputmask="'alias': 'date'" name="ngaythangchuyen" tabindex="-1" type="text" class="form-control" value="<?php echo doingay1($row['ngaythangchuyen']); ?>" id="ngaythangchuyen"/>
			</div>
</fieldset>
</div>
	<div class = "col-sm-12">
