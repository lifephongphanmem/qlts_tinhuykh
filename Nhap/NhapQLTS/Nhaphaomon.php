...
$(document).on("click", ".open-AddBookDialog", function (e) {
	...
	$('#sTTKK').load('ajax_haomon.php?id_a='+myBookId);
	$('#sTTKK1').load('ajax_haomonnam.php?id_a='+myBookId);
});
 </script>
 
		<hr />	
		<h3>Quản lý hao mòn tài sản</h3>		
...
<?php 	
	...
	{		
		...
		$sql = "Insert into tblhaomon(ngaythang,TTQLTS,namhaomon,phantram,sodu,sotien,madonvi) values('$ngaythang',$TTQLTS,$namhaomon,$phantram,$sodu,$sotien,'$ab')";								
...
//
$sql1 = "select TTQLTS,mataisan,tentaisan,tenchitiet,ngaynhap,namsanxuat,NUOCSX,ngaysudung,DTKV,DTXD,ngansach,nguonkhac,chitiethinhthai from tblqlts where madonvi = '$ab' and chitiethinhthai not like 'Nhà%' and chitiethinhthai not like 'Quyền sử dụng đất%' and chitiethinhthai not like 'Phương tiện vận tải%'";
$kq1 = mysqli_query($con,$sql1);
$soluong =0;$ngnh="";$ngsd="";$mamt="";   
if(mysqli_num_rows($kq1)>0){
	while($row=mysqli_fetch_array($kq1)){
            ...
            echo"<tr>";
				...
				echo"<td>$soluong</td>";		
                echo"<td>						
						<a data-id='$row[0]' title='Add this item' class='open-AddBookDialog btn btn-primary' href='#addBookDialog'>Hao mòn</a>							
						<a href=\"Nhapsuahaomon.php?id=$mamt\" class='btn btn-info '>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">        
		<form role='form' method='POST' action = 'Nhaphaomon.php' class='form-horizontal form-groups-bordered'>
							...
							<div class="form-group">
								...
								<div class="col-sm-8">
									<input name="namhaomon" type="text" class="form-control" id="field-3">
								</div>
							</div>	
							<div id = "sTTKK" class="form-group">
								<label for="field-4" class="col-sm-3 control-label">Phần trăm hao mòn:</label>
								<div class="col-sm-8">
									<input name="phantram" type="text" class="form-control" id="field-3">
								</div>
							</div>
							<div class="form-group">
								<label for="field-4" class="col-sm-3 control-label">Số dư hao mòn:</label>
								<div class="col-sm-8">
									<input name="sodu" type="text" class="form-control" data-mask="fdecimal" id="field-3">
								</div>
							</div>	
							<div id = "sTTKK1" class="form-group">
								<label for="field-4" class="col-sm-3 control-label">Số hao mòn năm:</label>
								<div class="col-sm-8">
									<input name="sotien" type="text" class="form-control" data-mask="fdecimal" id="field-3">
								</div>
							</div>	
