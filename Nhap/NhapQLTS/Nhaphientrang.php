...
<?php 	
	...
	{		
		...
		$sql = "Insert into tblhientrang(ngaythang,TTQLTS,phanloai,donvi,soluong,madonvi) values('$ngaythang',$TTQLTS,'$phanloai','$donvi',$soluong,'$ab')";								
...
//
$sql = "select TTQLTS,mataisan,tentaisan,tenchitiet,ngaynhap,namsanxuat,NUOCSX,ngaysudung,DTKV,DTXD,ngansach,nguonkhac,".
"chitiethinhthai from tblqlts where madonvi = '$ma[0]' and chitiethinhthai not like 'Nhà%' and".
 " chitiethinhthai not like 'Quyền sử dụng đất%' and chitiethinhthai not like 'Phương tiện vận tải%' and".
  " tblqlts.chitiethinhthai not like 'Vật kiến trúc%' and (nguonkhac+ngansach) <500000000";
$kq = mysqli_query($con,$sql);
...
if(mysqli_num_rows($kq)>0){
	while($row=mysqli_fetch_array($kq)){
			...
			echo "<tr"; if(ktra($row[0])) echo " style='color:red;'"; echo ">";
				...
				if(!ktthanhly($row[0]))
                echo"<td>						
						<a data-id='$row[0]' title='Add this item' class='open-AddBookDialog btn btn-primary' href='#addBookDialog'>Hiện trạng</a>							
					...
					echo"<td>						
						<a data-id='$row[0]' title='Tài sản đã bán, thanh lý hoặc điều chuyển' class='open-AddBookDialog btn btn-warning '>Hiện trạng</a>							
						<a href=\"Nhaphientrangsua.php?id=$mamt\" class='btn btn-info'>							
							Chi tiết
						</a>	
					</td>";
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">        
		<form role='form' method='POST' action = 'Nhaphientrang.php' class='form-horizontal form-groups-bordered'>
							...
							<div class="form-group">
								<label for="field-3" class="col-sm-3 control-label">Ngày nhập hiện trạng:</label>
								<div class="col-sm-8">
									<input data-inputmask="'alias': 'date'" name="ngaythang" type="text" class="form-control" id="field-3" value = "">
								</div>
							</div>
							<div class="form-group">
								<label for="field-4" class="col-sm-3 control-label">Phân loại sử dụng:</label>
								<div class="col-sm-8">
									<select name='phanloai' class='form-control'>									
									<option selected='selected' value='Kinh doanh'>Kinh doanh</option>
									<option selected='selected' value='Không KD'>Không kinh doanh</option>
									<option selected='selected' value='QLNN'>Quản lý nhà nước</option>
									<option selected='selected' value='HĐ khác'>Hoạt động khác</option>
