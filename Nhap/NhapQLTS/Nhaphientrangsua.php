...
?>
		...
		<a class="tieude" href="Nhaphientrang.php">
			Hiện trạng sử dụng
		</a> >>
		<a class="tieude" >
			Chi tiết hiện trạng sử dụng
		</a>
		</div>
		...
		<a class="tieude">
			Loại tài sản:
		</a> <?php echo $chitiethinhthai; ?> >>
		<a class="tieude"  >
			Tên tài sản:
		</a>	<?php echo  $tentaisan;?>	>>
		<a class="tieude">
			Tên chi tiết:
		</a> <?php echo  $tenchitiet;?>
...
<?php 	
	...
	{		
		...
		$sql = "Update tblhientrang set ngaythang='$ngaythang',phanloai='$phanloai',donvi='$donvi',soluong=$soluong where TThientrang =".$TThientrang;								
		$kq=mysqli_query($con,$sql);
	}	        	
?>		
...
function ktra($TThientrang)
{
	global $a;
	global $con;
	$sql="Select TTTS from Tblhientrang where TThientrang = $TThientrang and TTTS = '$a' group by TTTS";	
	$qrsql=mysqli_query($con,$sql);
	if(mysqli_num_rows($qrsql) <= 0)
		return true;		
	else
...
if(mysqli_num_rows($kq)>0){
	while($row=mysqli_fetch_array($kq)){				
			$ngay = doingay1($row[1]);
			$stt++;
			$dk = $row[0].">".$ngay.">".$row[2].">".$row[3].">".$row[4];
			$maxoa= $row[0].">".$a;
            echo"<tr>";
                echo"<td>$stt</td>";
                echo"<td>$ngay</td>";
				...
				if(ktra($row['TThientrang']))					
						echo "<a href=\"xoaht.php?id=$maxoa\" onclick=\"return confirm('Bạn muốn xóa hiện trạng sử dụng này?')\" class='btn btn-red'>							
							Xóa
						</a>";
					echo "</td>";
				}
				else
				{
					...
					if(ktra($row['TThientrang']))
						echo "<a  title='Tài sản đã bán, thanh lý hoặc điều chuyển' class='btn btn-warning '>						
							Xóa
						</a>";
					echo"</td>";
            echo"</tr>";
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel">Chỉnh sửa hiện trạng sử dụng</h4>
      </div>
      <div class="modal-body">	  
		<?php echo "<form role='form' method='POST' action = 'Nhaphientrangsua.php?id=$a' class='form-horizontal form-groups-bordered'>"; ?>					
							<div class="form-group">
								<label class="col-sm-3 control-label">Số tài sản:</label>
								<div class="col-sm-5">
							...
							<div class="form-group">								
								<label for="field-4" class="col-sm-3 control-label">ĐVT:</label>
								<div class="col-sm-5">
