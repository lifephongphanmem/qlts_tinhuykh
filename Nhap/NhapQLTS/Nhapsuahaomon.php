...
function ktthanhly()
{
	global $a;
	global $con;
	$sql = "Select * from tbldenghi where TTQLTS = '".$a."' and (hinhthuc ='Thanh lý' or hinhthuc ='Bán' or hinhthuc ='Điều chuyển')";	
	$qrsql = mysqli_query($con,$sql);
...
if(mysqli_num_rows($kq)>0){
	while($row=mysqli_fetch_array($kq)){	
            ...
            echo"<tr>";
				...
				{
					echo"<td>					
						<a data-id='$tatca' title='Tài sản đã bán, thanh lý hoặc điều chuyển' class='open-AddBookDialog btn btn-warning '>Sửa</a>";
						if(!ktra($row[0]))	
				...
				else
				{
					echo"<td>					
						...
						if(!ktra($row[0]))	
						echo "<a href=\"xoahm.php?id=$maxoa\" onclick=\"return confirm('Bạn muốn xóa hao mòn này?')\" class='btn btn-red '>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
		...
		<?php echo "<form role='form' method='POST' action = 'Nhapsuahaomon.php' class='form-horizontal form-groups-bordered'>"; ?>

                                 <input type='text' style="display: none" name='TTQLTS' id='TTQLTS' value="<?php echo $a;?>">
