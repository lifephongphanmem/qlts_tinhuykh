...
$(document).on("click", ".open-AddBookDialog", function (e) {
	...
	$("#ngaytanggiam").val(myBookId[1]);
	$("#lydotanggiam").val(myBookId[2]);
	$("#soluong").val(myBookId[3]);
    ...
    $(_self.attr('href')).modal('show');		
		...
		<div class="col-sm-12">
			...
			>><a class="tieude" href='NhapTang.php?id=<?php echo $_GET['id'];?>'>
				Tăng giảm sửa chữa
			</a>
			>><a class="tieude" >
		...
		<h3>Chỉnh sửa tăng giảm tài sản</h3>			
		<input type="button" class="btn btn-black" value="Trở lại" onclick="<?php echo "window.location.href='NhapTang.php'" ?>" />		
<?php 	
	...
	if (isset($_POST['capnhatchinhsua']))
	{
		...
		$sql = "Update tbltanggiam set ngaytanggiam='$ngaytanggiam',lydotanggiam='$lydotanggiam',soluong=$soluong,sotien=$sotien where TTTGTS =".$TTTGTS;								
...
//
$a = $_GET['id'];
$sql = "select TTTGTS,ngaytanggiam,tanggiam,lydotanggiam,soluong,sotien from tbltanggiam where TTQLTS = '$a'";
$kq = mysqli_query($con,$sql); 
$ngay = "";$tatca="";
if(mysqli_num_rows($kq)>0){
	while($row=mysqli_fetch_array($kq)){	
			$ngay = doingay1($row[1]);
			$tatca=$row[0].">".$ngay.">".$row[3].">".$row[4].">".$row[5];
			$maxoa = $row[0].">".$a;
            echo"<tr>";
                echo"<td>$row[0]</td>";
                echo"<td>$ngay</td>";
				...
				if(!ktthanhly($a))
                echo"<td>
						<a href=\"NhapTangsua.php?id=$row[0]\" class='btn btn-green' >							
								Sửa	
							</a>							
						<a href=\"xoatg.php?id=$maxoa\" onclick=\"return confirm('Bạn muốn xóa tài sản này?')\" class='btn btn-red'>							
							Xóa
						</a>						
					</td>";
				else
					echo"<td>
						<a title='Tài sản đã bán, thanh lý hoặc điều chuyển' class='btn btn-warning '>Sửa</a>						
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel">Chỉnh sửa tăng giảm tài sản</h4>
      </div>
      <div class="modal-body">
        
		<?php echo "<form role='form' method='POST' action = 'Nhapsuatanggiam.php?id=$a' class='form-horizontal form-groups-bordered'>"; ?>
							...
							<div id = "sTTKK1" class="form-group">
								<label for="field-3" class="col-sm-3 control-label">Ngày tháng:</label>
								<div class="col-sm-8">
							...
							<div id = "sTTKK2" class="form-group">
								<label for="field-4" class="col-sm-3 control-label">Lý do tăng giảm:</label>
								<div class="col-sm-8">
									<input name="lydotanggiam" type="text" class="form-control" id="lydotanggiam">
								</div>
							</div>	
							...
							<div id = "sTTKK3" class="form-group">
								<label for="field-4" class="col-sm-3 control-label">Số tiền:</label>
								<div class="col-sm-8">
