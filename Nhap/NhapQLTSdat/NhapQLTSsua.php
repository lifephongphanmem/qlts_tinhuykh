...
<script>
    ...
    $('#khauhao').click(function(){
		...
		$('#namsudungconlai').slideToggle('fast');
...
$(document).on("click", ".open-AddBookDialog", function (e) {		
	...
	if(myBookId[26] == "Theo dõi")
		document.getElementById("theodoics").checked = true;
	else
		document.getElementById("theodoics").checked = false;
...
$(document).on("click", ".open-AddBookDialog1", function (e) {	
	...
	if(myBookId[26] == "Theo dõi")
		document.getElementById("theodoic").checked = true;
	else
		document.getElementById("theodoic").checked = false;
    $(_self.attr('href')).modal('show');		
});
 </script>
 <script>
...
if(isset($_POST["luupb"]))
	...
	{		
		...
		$giaycnqsd=$_POST['giaycnqsd'];
		$ngaygiaycnqsd=doingay($_POST['ngaygiaycnqsd']);
		...
		$sql="update tblqlts set mataisan='$mataisan',tentaisan='$tentaisan',tenchitiet='$tenchitiet',ngaynhap='$ngaynhap',ngaysudung='$ngaysudung',noidung='$noidung',ngansach=$ngansach,nguonkhac=$nguonkhac,DTKV=$DTKV,TKVDAT='$TKVDAT',chitiethinhthai='$chitiethinhthai',XOA='K',DVT='m2',phantram=$pthm,sonamsd=$sonamsd,sothutu=$ttsx,theodoi='$theodoi',quyetdinhbangiao='$quyetdinhbangiao',ngayquyetdinhbangiao='$ngayquetdinhbangiao',giaycnqsd='$giaycnqsd',ngaygiaycnqsd='$ngaygiaycnqsd',ngaythangchuyen ='$ngaythangchuyen' where TTQLTS = $id";
		$kq=mysqli_query($con,$sql);		
	...
	$sql = "select tblqlts.mataisan,tblqlts.sonamsd,tblqlts.tenchitiet,tblqlts.ngaynhap,tblqlts.ngaysudung,tblqlts.noidung,tblqlts.TKVDAT,tblqlts.DTKV,tblqlts.ngansach,tblqlts.nguonkhac,tblqlts.quyetdinhbangiao,tblqlts.ngayquyetdinhbangiao,tblqlts.giaycnqsd,tblqlts.ngaygiaycnqsd,tblqlts.ngaythangchuyen FROM tblqlts WHERE tblqlts.TTQLTS = '$id'";
	$qrsql = mysqli_query($con,$sql);
	while($row = mysqli_fetch_array($qrsql))
	{	
?>
...
<div class="col-sm-12">		
  ...
  <legend>Thông tin tài sản</legend> 
			...
			<?php								
			$sql="select mataisanqd32,tentaiqd32,phantramhaomon,phantram32,namsudung from tbldanhsachqd32 Where (chitiethinhthai like 'Quyền sử dụng đất%' or chitiethinhthai like 'Đất%')  and mataisanqd32 Like 'TS%'";
			...
			<div class="col-sm-4">
				<input data-inputmask="'alias': 'date'" name="ngaynhap" value="<?php echo doingay1($row[3]); ?>" type="text" data-validate = "required" class="form-control" id="ngaynhap"/>
			</div>
			<div class="col-sm-05">
			</div>
			<label for="field-4" class="col-sm-15 control-label">Ngày sử dụng<span style="padding-left:0px; color:red;">*</span>:</label>
			<div class="col-sm-45">
				<input data-inputmask="'alias': 'date'" name="ngaysudung" value="<?php echo doingay1($row[4]); ?>" type="text" data-validate = "required" class="form-control" id="ngaysudung"/>
			</div>												
			...
			echo"</div>";
			?>	
			<a onclick="showAjaxModal();" class='col-sm-05 btn'><img src="/icon/add.png"  height="16" width="20"/></a>
			<label for="field-4" class="col-sm-15 control-label">Đ/c khuôn viên<span style="padding-left:0px; color:red;">*</span>:</label>
			<div class="col-sm-45">
				<input name="TKVDAT" type="text" data-validate = "required" value="<?php echo $row[6]; ?>" class="form-control" id="field-3">
			</div>
	...
	<fieldset>
	<legend>Diện tích, nguyên giá</legend>	
			<label for="field-4" class="col-sm-15 control-label">Diện tích:</label>
			<div class="col-sm-4">
				<input name="DTKV" type="text" value="<?php echo $row[7]; ?>" class="form-control" id="DTKV" />			
			</div>
			...
			<div class="col-sm-4">
				<input name="nguyengia" type="text" readonly tabindex="-1" value="<?php echo (kieudouble($row[8])+kieudouble($row[9])); ?>" class="form-control" data-mask="fdecimal" id="nguyengia"/>									
			</div>
			...
			<div class="col-sm-4">
				<input name="ngansach" type="text" value="<?php echo $row[8]; ?>" class="form-control" data-mask="fdecimal" id="ngansach"/>					
			</div>
			<label for="field-4" style="padding-left: 0px;" class="col-sm-05 control-label">(VNĐ)</label>
			<label for="field-3" class="col-sm-15 control-label">Nguồn khác:</label>
			<div class="col-sm-4">	
				<input name="nguonkhac" type="text" class="form-control" value="<?php echo $row[9]; ?>" data-mask="fdecimal" id="nguonkhac"/>			
			</div>	
			<label for="field-4" style="padding-left: 0px;" class="col-sm-05 control-label">(VNĐ)</label>
			</fieldset>	
</div>	
<div class="col-sm-6" style = "display:none">	
	...
	<legend>Hao mòn/khấu hao</legend>	
			...
			<label for="field-4"  class="col-sm-4 control-label">Hao mòn TT45:</label>
			<div class="col-sm-8">
				<input name="phantramhaomon" readonly tabindex="-1" type="text" class="form-control" id ="phantramhaomon"/>
			</div>
			<label for="field-4"  class="col-sm-4 control-label">Hao mòn QĐ32:</label>
			<div class="col-sm-8">
				<input name="phantram32"  type="text" readonly tabindex="-1" class="form-control" id ="phantram32"/>
			</div>
			<label for="field-4" class="col-sm-4 control-label">Số hao mòn:</label>
			<div class="col-sm-8">
				<input name="sohaomon" type="text" class="form-control" data-mask="fdecimal" id="sohaomon"/>
			</div>
			<label for="field-4" class="col-sm-4 control-label">Giá trị còn lại:</label>
			<div class="col-sm-8">
				<input name="conlai" type="text" class="form-control" data-mask="fdecimal" id="conlai"/>
			</div>
			<label id="lbngaykhauhao" for="field-4" style="display:none" class="col-sm-4 control-label">Thời gian tính KH:</label>
			<div class="col-sm-8">
				<input data-inputmask="'alias': 'date'" style="display:none" name="ngaykhauhao" type="text" class="form-control" id="ngaykhauhao"/>
			</div>
			<label for="field-4" id="lbnamsudungconlai" style="display:none" class="col-sm-4 control-label">Thời gian SD còn lại:</label>
			<div class="col-sm-8">
				<input name="namsudungconlai" style="display:none" type="text" class="form-control" id ="namsudungconlai"/>
			</div>								
			<label for="field-4" id="lbphuongphapkhauhao" style="display:none" class="col-sm-4 control-label">Phương pháp tính KH:</label>
			<div class="col-sm-8" >
			...
			<label for="field-4" id="lbthangkhauhaoconlai" style="display:none" class="col-sm-4 control-label">Thời gian KH còn lại:</label>
			<div class="col-sm-8">
				<input name="thangkhauhaoconlai" style="display:none" type="text" class="form-control" data-mask="fdecimal" id="thangkhauhaoconlai"/>
			</div>
			<label for="field-4" id="lbtylekhauhao" style="display:none" class="col-sm-4 control-label">Tỷ lệ khấu hao:</label>
			<div class="col-sm-8">
				<input name="tylekhauhao" style="display:none" type="text" class="form-control" data-mask="fdecimal" id="tylekhauhao"/>
			</div>
			<label for="field-4" id="lbtylesxkd" style="display:none" class="col-sm-4 control-label">Tỷ lệ SD SXKD-DV:</label>
			<div class="col-sm-8">
				<input name="tylesxkd" style="display:none" type="text" class="form-control" data-mask="fdecimal" id="tylesxkd"/>
			</div>
			<label for="field-4" id="lbsokhauhao" style="display:none" class="col-sm-4 control-label">Số đã khấu hao:</label>
			<div class="col-sm-8">
...
<div class="col-sm-12">	
		...
		<fieldset>
 		<legend>Thông tin khác</legend>					
			<input name="DVT" type="text" style="display:none;" class="form-control" id="field-3" value="m2"/>					
			<label for="field-4" class="col-sm-15 control-label">Giấy CNQSD đất:</label>
			<div class="col-sm-4">
				<input name="giaycnqsd" value="<?php echo $row[10]; ?>" type="text" class="form-control" id="field-3"/>				
			</div>
			<div class="col-sm-05">
			</div>
			<label for="field-4" class="col-sm-15 control-label">Ngày tháng:</label>
			<div class="col-sm-4">
				<input data-inputmask="'alias': 'date'" name="ngaygiaycnqsd" value="<?php echo doingay1($row[11]); ?>" type="text" class="form-control" id="ngaygiaycnqsd">
			</div>	
			
			<label for="field-4" class="col-sm-15 control-label">Quyết định giao đất:</label>
			<div class="col-sm-4">
				<input name="quyetdinhbangiao" type="text" value="<?php echo $row[12]; ?>" class="form-control" id="field-3"/>									
			</div>
			<div class="col-sm-05">
			</div>
			<label for="field-4" class="col-sm-15 control-label">Ngày tháng:</label>
			<div class="col-sm-4">
				<input data-inputmask="'alias': 'date'" value="<?php echo doingay1($row[13]); ?>" name="ngayquyetdinhbangiao" type="text" class="form-control" id="ngayquyetdinhbangiao">
			</div>				
		</fieldset>								
</div>		
