...
//
$sql = "select TTQLTS,mataisan,tentaisan,tenchitiet,ngaynhap,namsanxuat,NUOCSX,ngaysudung,DTKV,DTXD,ngansach,nguonkhac,chitiethinhthai,sonamsd from tblqlts where madonvi = '$ab' and chitiethinhthai like 'Nhà%'";
$kq = mysqli_query($con,$sql);
...
if(mysqli_num_rows($kq)>0){
	while($row=mysqli_fetch_array($kq)){
			...
			echo "<tr"; if(ktra($row[0])) echo " style='color:red;'"; echo ">";
                ...
                if(!ktthanhly($row[0]))				
                echo"<td>							
						<a data-id='$tangg' title='Add this item' class='open-AddBookDialog btn btn-primary' href='#addBookDialog'>Ghi tăng</a>	
						<a data-id='$tangg' title='Add this item' class='open-AddBookDialogg btn btn-primary' href='#addBookDialogg'>Ghi giảm</a>							
				 ...
				 echo"<td>							
						<a data-id='$tangg' title='Tài sản đã bán, thanh lý hoặc điều chuyển' onclick='getFocuscs()' class='open-AddBookDialog btn btn-warning '>Ghi tăng</a>	
						<a data-id='$tangg' title='Tài sản đã bán, thanh lý hoặc điều chuyển' onclick='getFocuscs()' class='open-AddBookDialog btn btn-warning '>Ghi giảm</a>							
						<a href=\"Nhapsuatanggiam.php?id=$mamt\" class='btn btn-info' >							
							Chi tiết	
						</a>	
					</td>";
