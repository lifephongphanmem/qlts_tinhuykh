...
function ktphanloai()
 ...
 function tinhtoan()
 {
	...
	if(soluong != "")
		...
		soluong = 0;
	var soluongcu = $("#dientichtruoc").val();
	if(soluongcu != "")
		soluongcu = soluongcu.replace(/,/g,"");
	else
		soluongcu = 0;		
	if(tanggiam == "Tăng")			
		var tongtang = parseFloat(soluong) + parseFloat(soluongcu);
	if(tanggiam == "Giảm")	
		var tongtang = parseFloat(soluongcu) - parseFloat(soluong);
	$("#soluongmoi").val(tongtang);
	...
	if(tanggiam == "Tăng")			
		var tongtang = nguyengiamoi + parseFloat(nguyengiatruoc);
	if(tanggiam == "Giảm")	
		var tongtang = parseFloat(nguyengiatruoc) - nguyengiamoi;
	$("#sotienmoi").val(tongtang);	
...
if(isset($_POST["luupb"]))
	...
	{			
		...
		"nguonkhac,nguyengiatruoc,ngansach,dientichtruoc)".
		" values('$ngaytanggiam',$id,'$lydotanggiam',$soluong,$sotien,'$tanggiam','$ab',".
		"$nguonkhac,$nguyengiatruoc,$ngansach,$dientichtruoc)";				
		$kq=mysqli_query($con,$sql);	
		$IDTG=IDTG();
			$sql = "Insert into tblhientrang(ngaythang,TTQLTS,phanloai,donvi,soluong,TTTM) values('$ngaytanggiam',$id,'Trụ sở làm việc','m2',".kieudouble($_POST['tslv']).",$IDTG)";								
			$kq=mysqli_query($con,$sql);	
			$sql = "Insert into tblhientrang(ngaythang,TTQLTS,phanloai,donvi,soluong,TTTM) values('$ngaytanggiam',$id,'Cơ sở HĐ sự nghiệp','m2',".kieudouble($_POST['hdsn']).",$IDTG)";								
			$kq=mysqli_query($con,$sql);	
			$sql = "Insert into tblhientrang(ngaythang,TTQLTS,phanloai,donvi,soluong,TTTM) values('$ngaytanggiam',$id,'Làm nhà ở','m2',".kieudouble($_POST['nhao']).",$IDTG)";								
			$kq=mysqli_query($con,$sql);	
			$sql = "Insert into tblhientrang(ngaythang,TTQLTS,phanloai,donvi,soluong,TTTM) values('$ngaytanggiam',$id,'Cho thuê','m2',".kieudouble($_POST['chot']).",$IDTG)";								
			$kq=mysqli_query($con,$sql);	
			$sql = "Insert into tblhientrang(ngaythang,TTQLTS,phanloai,donvi,soluong,TTTM) values('$ngaytanggiam',$id,'Bỏ trống','m2',".kieudouble($_POST['bot']).",$IDTG)";								
			$kq=mysqli_query($con,$sql);	
			$sql = "Insert into tblhientrang(ngaythang,TTQLTS,phanloai,donvi,soluong,TTTM) values('$ngaytanggiam',$id,'Bị lấn chiếm','m2',".kieudouble($_POST['bic']).",$IDTG)";								
			$kq=mysqli_query($con,$sql);	
			$sql = "Insert into tblhientrang(ngaythang,TTQLTS,phanloai,donvi,soluong,TTTM) values('$ngaytanggiam',$id,'Khác','m2',".kieudouble($_POST['khac']).",$IDTG)";								
			$kq=mysqli_query($con,$sql);
	...
	$sql = "select tblqlts.tenchitiet,tblqlts.ngaynhap,tblqlts.ngaysudung,tblqlts.noidung,tblqlts.TKVDAT,tblqlts.DTKV,tblqlts.ngansach,tblqlts.nguonkhac,tblqlts.quyetdinhbangiao,tblqlts.ngayquyetdinhbangiao,tblqlts.giaycnqsd,tblqlts.ngaygiaycnqsd FROM tblqlts WHERE tblqlts.TTQLTS = '$id'";
	$qrsql = mysqli_query($con,$sql);
	while($row = mysqli_fetch_array($qrsql))
	{		
?>
	...
	<fieldset>
		<legend>Thông tin tài sản</legend> 	
			...
			<div class="col-sm-105">
				<input type="text" readonly tabindex="-1"  value="<?php echo $row["TKVDAT"]; ?>" class="form-control" id="field-3">
			</div>
		</fieldset>	
		<fieldset id="ttts">
			...
			<div class="col-sm-4">
				<input type="text" id="dientichtruoc" name ="dientichtruoc" readonly tabindex="-1" value="<?php echo $row["DTKV"]; ?>" class="form-control"  />			
			</div>
			...
			<fieldset class="col-sm-6">
				...
				<div class="col-sm-8">
					<input name="soluong" type="text" onblur="nguyengiasoluong()" class="form-control" data-mask="fdecimal" id="soluong" />			
				</div>								
				<label for="field-4" style="padding-left: 0px;" class="col-sm-05 control-label">(m2)</label>
			...
			<fieldset >
				...
				<label for="field-4" class="col-sm-3 control-label">Diện tích mới:</label>
				<div class="col-sm-8">
					<input name="soluongmoi" readonly tabindex="-1" type="text"  class="form-control" data-mask="fdecimal" id="soluongmoi" />			
				</div>
				<label for="field-4" style="padding-left: 0px;" class="col-sm-05 control-label">(m2)</label>
		...
		<fieldset>
 		<legend>Hiện trạng sử dụng</legend>							
			...
			<div class="col-sm-15">
				<input class="form-control" onblur="hientrang()" name="tslv" id="tslv" value="0" data-mask="fdecimal" type="text">
			</div>			
			<div class="col-sm-2">
				<input class="form-control" onblur="hientrang()" name="hdsn" id="hdsn" data-mask="fdecimal" value="0" type="text" >
			</div>			
			<div class="col-sm-15">
				<input class="form-control" onblur="hientrang()" name="nhao" id="nhao" data-mask="fdecimal" value="0" type="text" >
			</div>			
			<div class="col-sm-15">
				<input class="form-control" onblur="hientrang()" name="chot" id="chot" data-mask="fdecimal" value="0" type="text" >
			</div>			
			<div class="col-sm-15">
				<input class="form-control" onblur="hientrang()" name="bot" id="bot" data-mask="fdecimal" value="0" type="text" >
			</div>			
			<div class="col-sm-15">
				<input class="form-control" onblur="hientrang()" name="bic" id="bic" data-mask="fdecimal" value="0" type="text" >
			</div>			
			<div class="col-sm-15">
				<input class="form-control" onblur="hientrang()" name="khac" id="khac" data-mask="fdecimal" value="0" type="text" >
			</div>					
		</fieldset>										
</div>					
