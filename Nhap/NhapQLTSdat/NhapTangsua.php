...
function hientrang ($hientrang)
{
	...
	{			
		...
		"nguonkhac=$nguonkhac,nguyengiatruoc=$nguyengiatruoc,ngansach=$ngansach,dientichtruoc=$dientichtruoc where TTTGTS= $id";			
		$kq=mysqli_query($con,$sql);		
			$sql = "Update tblhientrang set ngaythang='$ngaytanggiam',donvi='m2',soluong=".kieudouble($_POST["tslv"])." where TTTM =$id and phanloai='Trụ sở làm việc'";											
			$kq=mysqli_query($con,$sql);		
			$sql = "Update tblhientrang set ngaythang='$ngaytanggiam',donvi='m2',soluong=".kieudouble($_POST["hdsn"])." where TTTM =$id and phanloai='Cơ sở HĐ sự nghiệp'";											
			$kq=mysqli_query($con,$sql);		
			$sql = "Update tblhientrang set ngaythang='$ngaytanggiam',donvi='m2',soluong=".kieudouble($_POST["nhao"])." where TTTM =$id and phanloai='Làm nhà ở'";											
			$kq=mysqli_query($con,$sql);		
			$sql = "Update tblhientrang set ngaythang='$ngaytanggiam',donvi='m2',soluong=".kieudouble($_POST["chot"])." where TTTM =$id and phanloai='Cho thuê'";											
			$kq=mysqli_query($con,$sql);		
			$sql = "Update tblhientrang set ngaythang='$ngaytanggiam',donvi='m2',soluong=".kieudouble($_POST["bot"])." where TTTM =$id and phanloai='Bỏ trống'";											
			$kq=mysqli_query($con,$sql);		
			$sql = "Update tblhientrang set ngaythang='$ngaytanggiam',donvi='m2',soluong=".kieudouble($_POST["bic"])." where TTTM =$id and phanloai='Bị lấn chiếm'";											
			$kq=mysqli_query($con,$sql);		
			$sql = "Update tblhientrang set ngaythang='$ngaytanggiam',donvi='m2',soluong=".kieudouble($_POST["khac"])." where TTTM =$id and phanloai='Khác'";
			$kq=mysqli_query($con,$sql);
				header("Location: Nhaptang.php");
	}	
	$sql = "select tblqlts.TTQLTS,tblqlts.tenchitiet,tblqlts.ngaynhap,tblqlts.ngaysudung,tblqlts.noidung,tblqlts.TKVDAT,tblqlts.DTKV,tblqlts.ngansach,tblqlts.nguonkhac,tbltanggiam.TTTGTS,tbltanggiam.ngaytanggiam,tbltanggiam.lydotanggiam,tbltanggiam.soluong,tbltanggiam.sotien,tbltanggiam.nguonkhac,tbltanggiam.nguyengiatruoc,tbltanggiam.ngansach,tbltanggiam.dientichtruoc,tbltanggiam.tanggiam FROM tblqlts INNER JOIN tbltanggiam ON tblqlts.TTQLTS = tbltanggiam.TTQLTS WHERE tbltanggiam.TTTGTS = '$id'";
	$qrsql = mysqli_query($con,$sql);
	while($row = mysqli_fetch_array($qrsql))
	{			
?>
...
<div class="col-sm-12">	
	...
	<fieldset>
		...
		<fieldset id="ttts">
			...
			<div class="col-sm-4">
				<input type="text" id="dientichtruoc" name ="dientichtruoc" readonly tabindex="-1" value="<?php echo $row["dientichtruoc"]; ?>" class="form-control"  />			
			</div>
			...
			<fieldset class="col-sm-6">
				...
				<div class="col-sm-8">
					<input name="soluong" value="<?php echo $row["soluong"]; ?>" type="text" onblur="nguyengiasoluong()" class="form-control" data-mask="fdecimal" id="soluong" />			
				</div>								
				<label for="field-4" style="padding-left: 0px;" class="col-sm-05 control-label">(m2)</label>
