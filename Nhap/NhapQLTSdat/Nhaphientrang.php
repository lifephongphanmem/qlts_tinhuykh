...
<script>	
	function tinhtoan()
	{
		var tongdientich = $("#DTKV").val();
		if(tongdientich != "")
			tongdientich = tongdientich.replace(/,/g,"");
		else
			tongdientich = 0;	
		var trusolv = $("#tslv").val();
		if(trusolv != "")
			trusolv = trusolv.replace(/,/g,"");
		else
			trusolv = 0;
		var hdsn = $("#hdsn").val();
		if(hdsn != "")
		...
		if(khac != "")
			...
			khac = 0;
		var kq=0;
		kq = parseFloat(tongdientich) - parseFloat(trusolv) - parseFloat(hdsn) - parseFloat(nhao) - parseFloat(chot) - parseFloat(bot) - parseFloat(bic) - parseFloat(khac);
		$("#DTKVm").val(kq);
	}
	$(document).ready(function()
{
	$('#tslv').change(function(){		
		tinhtoan();
	});
});
$(document).ready(function()
{
	$('#hdsn').change(function(){		
		tinhtoan();
	});
});
$(document).ready(function()
{
	$('#nhao').change(function(){		
		tinhtoan();
	});
});
$(document).ready(function()
{
	$('#chot').change(function(){		
		tinhtoan();
	});
});
$(document).ready(function()
...
$(document).ready(function()
{
	$('#khac').change(function(){		
		tinhtoan();
	});
});
function getFocus() {
    document.getElementById("tslv").focus();
	document.getElementById("hdsn").focus();
	document.getElementById("nhao").focus();
	document.getElementById("chot").focus();
	document.getElementById("bic").focus();
	document.getElementById("khac").focus();
...
$(document).on("click", ".open-AddBookDialog", function (e) {
	...
	$("#DTKV").val(myBookId[1]);
	$("#DTKVm").val(myBookId[1]);
    $(_self.attr('href')).modal('show');	
});
 </script>
 
		<hr />	
		<h3>Quản lý hiện trạng sử dụng tài sản</h3>
...
//		$soluong=kieudouble($_POST["soluong"]);	
			...
			$sql = "Insert into tblhientrang(ngaythang,TTQLTS,phanloai,donvi,soluong,madonvi) values('$ngaythang',$TTQLTS,'Trụ sở làm việc','$donvi',".kieudouble($_POST['tslv']).",'$ab')";								
			$kq=mysqli_query($con,$sql);	
			$sql = "Insert into tblhientrang(ngaythang,TTQLTS,phanloai,donvi,soluong,madonvi) values('$ngaythang',$TTQLTS,'Cơ sở HĐ sự nghiệp','$donvi',".kieudouble($_POST['hdsn']).",'$ab')";								
			$kq=mysqli_query($con,$sql);	
			$sql = "Insert into tblhientrang(ngaythang,TTQLTS,phanloai,donvi,soluong,madonvi) values('$ngaythang',$TTQLTS,'Làm nhà ở','$donvi',".kieudouble($_POST['nhao']).",'$ab')";								
			$kq=mysqli_query($con,$sql);	
			$sql = "Insert into tblhientrang(ngaythang,TTQLTS,phanloai,donvi,soluong,madonvi) values('$ngaythang',$TTQLTS,'Cho thuê','$donvi',".kieudouble($_POST['chot']).",'$ab')";								
			$kq=mysqli_query($con,$sql);	
			$sql = "Insert into tblhientrang(ngaythang,TTQLTS,phanloai,donvi,soluong,madonvi) values('$ngaythang',$TTQLTS,'Bỏ trống','$donvi',".kieudouble($_POST['bot']).",'$ab')";								
			$kq=mysqli_query($con,$sql);	
			$sql = "Insert into tblhientrang(ngaythang,TTQLTS,phanloai,donvi,soluong,madonvi) values('$ngaythang',$TTQLTS,'Bị lấn chiếm','$donvi',".kieudouble($_POST['bic']).",'$ab')";								
			$kq=mysqli_query($con,$sql);	
			$sql = "Insert into tblhientrang(ngaythang,TTQLTS,phanloai,donvi,soluong,madonvi) values('$ngaythang',$TTQLTS,'Khác','$donvi',".kieudouble($_POST['khac']).",'$ab')";								
			$kq=mysqli_query($con,$sql);	
	}	        	
?>		
...
function ktra($mats)
{
	$kq="";
	global $con;
	$sql="select * from tblhientrang where TTQLTS = ".$mats;	
...
//
$sql = "select TTQLTS,mataisan,tentaisan,tenchitiet,ngaynhap,ngaysudung,DTKV,ngansach,nguonkhac,TKVDAT,noidung from tblqlts where madonvi = '$ab' and (chitiethinhthai like 'Đất%' or chitiethinhthai like 'Quyền sử dụng đất%')";
$kq = mysqli_query($con,$sql);
$ngnh="";$ngsd="";$mamt="";   $stt=0;  
if(mysqli_num_rows($kq)>0){
	while($row=mysqli_fetch_array($kq)){		
		$stt++;
		$mamt = $row[0];	
		$hientrang = $row[0].">".$row[6];
            echo "<tr"; if(ktra($row[0])) echo " style='color:red;'"; echo ">";
				...
				echo"<td>$row[6]</td>";		
                echo"<td>						
						<a data-id='$hientrang' title='Add this item' class='open-AddBookDialog btn btn-primary' href='#addBookDialog'>Hiện trạng</a>							
						<a href=\"Nhaphientrangsua.php?id=$mamt\" class='btn btn-info'>							
							Chi tiết
						</a>	
					</td>";
            echo"</tr>";
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" style="text-align:center;" id="myModalLabel"><strong>Thêm mới hiện trạng sử dụng tài sản</strong></h4>
      </div>
      <div class="modal-body">        
		<form role='form' action = 'Nhaphientrang.php' method='POST' class='form-horizontal form-groups-bordered'>
							...
							<div class="form-group">
								<label for="field-3" class="col-sm-3 control-label">Ngày nhập:</label>
								<div class="col-sm-8">
									<input data-inputmask="'alias': 'date'" name="ngaythang" type="text" class="form-control" id="field-3" value = "">
								</div>							
								<label for="field-4" class="col-sm-3 control-label">Đơn vị tính:</label>
								<div class="col-sm-8">
									<input name="donvi" type="text" class="form-control" id="field-3" value = "m2">
								</div>
							</div>
							<div class="form-group">
								<h4> Phân loại sử dụng </h4>
								<label for="field-4" class="col-sm-3 control-label">Tổng diện tích:</label>
								<div class="col-sm-8">
									<input class="form-control" style="display:none" name="DTKV" id="DTKV" data-mask="fdecimal" type="text">
									<input class="form-control" readonly name="DTKVm" id="DTKVm" data-mask="fdecimal" type="text">
								</div>
								<label for="field-4" class="col-sm-3 control-label">Trụ sở làm việc:</label>
								<div class="col-sm-8">
									<input class="form-control" name="tslv" id="tslv" data-mask="fdecimal" type="text">
								</div>
								<label for="field-4" class="col-sm-3 control-label">CSHĐ sự nghiệp:</label>
								<div class="col-sm-8">
									<input class="form-control" name="hdsn" id="hdsn" data-mask="fdecimal" type="text">
								</div>
								<label for="field-4" class="col-sm-3 control-label">Làm nhà ở:</label>
								<div class="col-sm-8">
									<input class="form-control" name="nhao" id="nhao" data-mask="fdecimal" type="text">
								</div>
								<label for="field-4" class="col-sm-3 control-label">Cho thuê:</label>
								<div class="col-sm-8">
									<input class="form-control" name="chot" id="chot" data-mask="fdecimal" type="text">
								</div>
								<label for="field-4" class="col-sm-3 control-label">Bỏ trống:</label>
								<div class="col-sm-8">
									<input class="form-control" name="bot" id="bot" data-mask="fdecimal" type="text">
								</div>
								<label for="field-4" class="col-sm-3 control-label">Bị lấn chiếm:</label>
								<div class="col-sm-8">
									<input class="form-control" name="bic" id="bic" data-mask="fdecimal" type="text">
								</div>
								<label for="field-4" class="col-sm-3 control-label">Khác:</label>
								<div class="col-sm-8">
									<input class="form-control" name="khac" id="khac" data-mask="fdecimal" type="text">
								</div>
								<!--
								<div class="col-sm-8">
							...
							<div class="modal-footer">
								...
								<input type="submit" onclick = "getFocus()" name="capnhatchinhsua" class="btn btn-blue" value = "Cập nhật">
							</div>		
						</form>
      </div>      
    </div><!-- /.modal-content -->
