...
function tongdt()
{
    ...
    var sotang = $("#SOTA").val();
    var tongdientich = parseFloat(sotang) * parseFloat(dientich);
...
if(isset($_POST["luupb"]))
	...
	if (isset($_POST['capnhatts']))
	{
		...
		for($i = 0; $i < $socopy; $i++){
		$sql="INSERT INTO tblqlts(mataisan,tentaisan,tenchitiet,CAPH,SOTA,DTXD,TDTSXD,namsanxuat,noidung,ngaynhap,ngaysudung,ngansach,nguonkhac,TKVDAT,phantram,sonamsd,sothutu,XOA,DVT,madonvi,chitiethinhthai,phantram32,sohaomon,conlai,xemt,xemh,theodoi,quyetdinhbangiao,ngayquyetdinhbangiao,bienbannghiemthu,ngaybienbannghiemthu,lydotang,giatrikhcl,khauhao,namsudungcl,ngaythangchuyen)";
		$sql = $sql ." Values('$mataisan','$tentaisan','$tenchitiet','$CAPH','$SOTA','$DTXD','$TDTSXD','$namsanxuat','$noidung','$ngaynhap','$ngaysudung',$ngansach,$nguonkhac,'$TKVDAT',$pthm,$sonamsd,$ttsx,'K','m2','$ab','$chitiethinhthai','$phantram32',$sohaomon,$conlai,'Chưa xem','Chưa xem','$theodoi','$quyetdinhbangiao','$ngayquetdinhbangiao','$bienbannghiemthu','$ngaybienbannghiemthu','$lydotang',$giatrikhcl,'$khauhao','$namsudungcl','$ngaythangchuyen')";
		$kq=mysqli_query($con,$sql);	
		$IDTS = IDTS();		
