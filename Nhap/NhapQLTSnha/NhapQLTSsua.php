...
function caphang()
{
    var tentaisan = $("#mataisanth").val();
    tentaisan = tentaisan.split('>');
    switch (tentaisan){
        case "TSCD.001":
            $("#CAPH").val("");
            break;
...
function tongdt()
{
    var dientich = $("#DTXD").val();
    var sotang  = $("#SOTA").val();
    var tongdientich = parseFloat(dientich) * parseFloat(sotang);
    $("#TDTSXD").val(tongdientich);
}
$(document).ready(function(){
    $("#SOTA").change(function(){
        tongdt();
    });
});
$(document).ready(function(){
    $("#DTXD").change(function(){
        tongdt();
    });
});
</script>
...
if(isset($_POST["luupb"]))
	...
	if (isset($_POST['capnhatts']))
	{
		...
		$sql="update tblqlts set mataisan='$mataisan',tentaisan='$tentaisan',tenchitiet='$tenchitiet',CAPH='$CAPH',SOTA='$SOTA',DTXD='$DTXD',TDTSXD='$TDTSXD',namsanxuat='$namsanxuat',noidung='$noidung',ngaynhap='$ngaynhap',ngaysudung='$ngaysudung',ngansach=$ngansach,nguonkhac=$nguonkhac,TKVDAT='$TKVDAT',phantram=$pthm,sonamsd=$sonamsd,sothutu=$ttsx,XOA='K',DVT='m2',chitiethinhthai='$chitiethinhthai',phantram32='$phantram32',sohaomon=$sohaomon,conlai=$conlai,theodoi='$theodoi',quyetdinhbangiao='$quyetdinhbangiao',ngayquyetdinhbangiao='$ngayquetdinhbangiao',bienbannghiemthu='$bienbannghiemthu',ngaybienbannghiemthu='$ngaybienbannghiemthu',lydotang = '$lydotang',giatrikhcl=$giatrikhcl,khauhao='$khauhao',namsudungcl=$namsudungcl,ngaythangchuyen = '$ngaythangchuyen'  where TTQLTS = $id";
		$kq=mysqli_query($con,$sql);	
...
<div class="col-sm-12">		
  ...
  <legend>Thông tin tài sản</legend> 
			...
			<?php								
			$sql="select mataisanqd32,tentaiqd32,phantramhaomon,phantram32,namsudung from tbldanhsachqd32 Where  (chitiethinhthai like 'Nhà%' or hinhthaitaisan like 'Nhà%')  and mataisanqd32 Like 'TS%'";
			...
			<label for="field-4" class="col-sm-15 control-label">Cấp hạng<span style="padding-left:0px; color:red;">*</span>:</label>
			<div class="col-sm-45">									
				<select name='CAPH' class='form-control' id="CAPH">
				<option <?php if($row[3] == "Cấp I") echo "selected='selected'" ; ?> value='Cấp I'>Cấp I</option>
				<option <?php if($row[3] == "Cấp II") echo "selected='selected'" ; ?> value='Cấp II'>Cấp II</option>
				<option <?php if($row[3] == "Cấp III") echo "selected='selected'" ; ?> value='Cấp III'>Cấp III</option>
				<option <?php if($row[3] == "Cấp IV") echo "selected='selected'" ; ?> value='Cấp IV'>Cấp IV</option>	
				<option <?php if($row[3] == "") echo "selected='selected'" ; ?>  value=''>Chọn cấp hạng nhà</option>				
				</select>
			</div>	
			...
			echo"</div>";
			?>	
            <a onclick="showAjaxModal();" class='col-sm-05 btn'><img src="/icon/add.png"  style="padding-left: 0px;" height="16" width="16"/></a>
			...
			<label for="field-3" class="col-sm-15 control-label">Số tầng<span style="padding-left:0px; color:red;">*</span>:</label>
			<div class="col-sm-45">
				<input name="SOTA" type="text" data-validate = "required" value="<?php echo $row[9]; ?>" class="form-control" id="SOTA">
			</div>
			<label for="field-3" class="col-sm-15 control-label">Diện tích<span style="padding-left:0px; color:red;">*</span>:</label>
			<div class="col-sm-4">
				<input name="DTXD" type="text" data-validate = "required" value="<?php echo $row[10]; ?>" class="form-control" id="DTXD">
			</div>
			<label for="field-4" style="padding-left: 0px;" class="col-sm-05 control-label">(m2)</label>
			<label for="field-3" class="col-sm-15 control-label">Tổng DT sàn<span style="padding-left:0px; color:red;">*</span>:</label>
			<div class="col-sm-4">
				<input name="TDTSXD" type="text" data-validate = "required" value="<?php echo $row[11]; ?>" class="form-control" id="TDTSXD">
			</div>
			<label for="field-4" style="padding-left: 0px;" class="col-sm-05 control-label">(m2)</label>
			...
			<div class="col-sm-4">
				<input name="nguyengia" type="text" readonly tabindex="-1" class="form-control" data-mask="fdecimal" value="<?php echo (kieudouble($row[13])+kieudouble($row[12])); ?>" id="nguyengia"/>
			</div>
			<label for="field-4" style="padding-left: 0px;" class="col-sm-05 control-label">(VNĐ)</label>
