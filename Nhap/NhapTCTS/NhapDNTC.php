...
$(document).on("click", ".open-AddBookDialog", function (e) {	
    ...
    $(_self.attr('href')).modal('show');		
		...
		<hr />
		<h3>Chọn tài sản trang cấp tài sản</h3>		
<?php		
		echo "<a href='NhapMoi.php' class='button'>
			Tạo mới
		</a>";
?>		
<?php 	
	function doingay1($ngay)
	{
		...
		{
			$ngay = doingay($_POST['ngaythang']);
			$up = "UPDATE tbltrangcap SET tengoi='$_POST[tengoi]',caphang='$_POST[caphang]',ngaythang='$ngay',hinhthuc='$_POST[hinhthuc]',soluong='$_POST[soluong]',sotien=".kieudouble($_POST['sotien'])." WHERE TTcap = ".$_POST['bookId'];
...
//
$ab = $ma[0];
$sql = "select TTcap,chitiethinhthai,mataisan,tentaisan,tengoi,caphang,ngaythang,hinhthuc,soluong,sotien,tinhtrang,lydo,chuyents from tbltrangcap where madonvi = '$ab'";
$kq = mysqli_query($con,$sql);
$tatca = "";
if(mysqli_num_rows($kq)>0){
	while($row=mysqli_fetch_array($kq)){
	$tatca = $row[0].">".$row[1].">".$row[2].">".$row[3].">".$row[4].">".$row[5].">".doingay1($row[6]).">".$row[7].">".$row[8].">".$row[9];
	$xoa =  $row[0].">".$ma[0];
            echo"<tr>";
                echo"<td>$row[0]</td>";
                echo"<td>$row[10]</td>";
				...
				echo"<td>$row[8]</td>";
                echo"<td>
						<a data-id='$tatca' title='Add this item' class='btn btn-blue ' href='NhapSua.php?id=$row[0]'>
							Sửa</a>
						<a href=\"xoa.php?id=$xoa\" onclick=\"return confirm('Bạn muốn xóa đề nghị trang cấp này?')\" class='btn btn-red '>
							Xóa
						</a>";
						if($row['tinhtrang'] == "Đã duyệt") {
							if ($row['chuyents'] != "Đã chuyển")
								echo " <a href=\"ChuyenTS.php?id=$xoa\" onclick=\"return confirm('Bạn muốn chuyển đề nghị trang cấp này sang bên quản lý tài sản?')\" class='btn btn-red '>
							Chuyển QLTS
							</a>";
							if ($row['chuyents'] == "Đã chuyển")
								echo " <a title='Tài sản đã được chuyển sang bên quản lý' class='btn btn-warning '>
							Chuyển QLTS
							</a>";
						}
					echo "</td>";
