...
function doingay1($ngay)
{
	...
	$id = $_GET['id'];
	$sql = "Select mataisan,ngaythang,tengoi,caphang,hinhthuc,soluong,sotien,madonvi,tentaisan,chitiethinhthai,tinhtrang,lydo,tendv,nguonvon,phuongthuctt,diadiem,dexuat,donvitinh,ghichu From tbltrangcap where TTcap = ".$id;
	...
	while($row = mysqli_fetch_array($qrsql)) {
		...
		<div class="col-sm-12">
			<form name="nhapmoi" onsubmit="return kt()" method="POST" class="form-horizontal form-wizard validate">
				<div class="col-sm-12">
					<fieldset>
						<?php echo $row['mataisan'];?>

						<legend>Thông tin tài sản</legend>
						<label class="col-sm-2 control-label">Tên tài sản<span
								style="padding-left:0px; color:red;">*</span>:</label>
						<?php
						$sqlts = "select mataisanqd32,tentaiqd32 from tbldanhsachqd32 where mataisanqd32 like 'TS%'";
						$qrsqlts = mysqli_query($con, $sqlts);
						...
						while ($data = mysqli_fetch_array($qrsqlts)) {

							if($row['mataisan'] == $data['mataisanqd32'])
						...
						echo "</div>";
						?>
						<label for="field-3" class="col-sm-2 control-label">Tên chi tiết<span
								style="padding-left:0px; color:red;">*</span>:</label>
						<div class="col-sm-4">
							<input name="tengoi" type="text" data-validate="required" class="form-control"
								   id="tengoi" value="<?php echo $row['tengoi'];?>"/>
						</div>
						<label for="field-3" class="col-sm-2 control-label">Cấp hạng, chủng loại<span
								style="padding-left:0px; color:red;">*</span>:</label>
						<div class="col-sm-4">
							<input name="caphang" type="text" data-validate="required" class="form-control"
								   id="caphang" value="<?php echo $row['caphang'];?>"/>
						</div>
						<label for="field-4" class="col-sm-2 control-label">Ngày nhập<span
								style="padding-left:0px; color:red;">*</span>:</label>
						<div class="col-sm-4">
							<input data-inputmask="'alias': 'date'" name="ngaythang" tabindex="-1" type="text"
								   data-validate="required" class="form-control" value="<?php echo doingay1($row['ngaythang']);?>" id="ngaythang"/>
						</div>
					</fieldset>
				</div>
				<div class="col-sm-12">
					<fieldset>
						...
						<div class="col-sm-4">
							<input name="soluong" type="text" class="form-control" value="<?php echo $row['soluong'];?>" id="soluong">
						</div>
						<label for="field-6" class="col-sm-2 control-label">Dự toán:</label>
						<div class="col-sm-4">
							<input name="sotien" type="text" class="form-control" data-mask="fdecimal" value="<?php echo $row['sotien'];?>" id="sotien">
						</div>
					</fieldset>
				</div>
				<div class="col-sm-12">
					<fieldset>
						...
						<label for="field-6" class="col-sm-2 control-label">Đơn vị tính:</label>
						<div class="col-sm-4">
							<input class="form-control" name="donvitinh" value="<?php echo $row['donvitinh'];?>" data-validate="required">
						</div>
						...
						<div class="col-sm-4">
							<select name="hinhthuc" type="text" class="form-control" id="hinhthuc">
								<option <?php if($row['hinhthuc'] == "Mua sắm") echo 'selected';?> value='Mua sắm'>Mua sắm</option>
								<option <?php if($row['hinhthuc'] == "Tiếp nhận") echo 'selected';?> value='Tiếp nhận'>Tiếp nhận</option>
							</select>
						</div>
						<label for="field-4" class="col-sm-2 control-label">Nguồn vốn:</label>
						<div class="col-sm-4">
							<input name="nguonvon" type="text" class="form-control" value="<?php echo $row['nguonvon'];?>" id="field-3"/>
						</div>
						<label for="field-4" class="col-sm-2 control-label">Phương thức TT:</label>
						<div class="col-sm-4">
							<input name="phuongthuctt" type="text" class="form-control" value="<?php echo $row['phuongthuctt'];?>" id="field-3"/>
						</div>
						<label for="field-4" class="col-sm-2 control-label">Địa điểm:</label>
						<div class="col-sm-4">
							<input name="diadiem" type="text" class="form-control" value="<?php echo $row['diadiem'];?>" id="field-3"/>
						</div>
						<label for="field-4" class="col-sm-2 control-label">Đề xuất:</label>
						<div class="col-sm-4">
							<input name="dexuat" type="text" class="form-control" value="<?php echo $row['dexuat'];?>" id="field-3"/>
						</div>
						<label for="field-4" class="col-sm-2 control-label">Ghi chú:</label>
						<div class="col-sm-10">
							<input name="ghichu" type="text" class="form-control" value="<?php echo $row['ghichu'];?>" id="field-3"/>
						</div>
					</fieldset>
				</div>
...
if (isset($_POST['save']))
{
	...
	$sql="Update tbltrangcap set mataisan='$mataisan',tentaisan='$tentaisan',tengoi='$tengoi',chitiethinhthai='$chitiethinhthai'".
		",caphang='$caphang',ngaythang='$ngaythang',hinhthuc='$hinhthuc',soluong=$soluong,sotien=$sotien,madonvi='$ab',tendv='$ma[2]'".
		",tinhtrang='Chờ duyệt',xemt='Chưa xem',xemh='Chưa xem',nguonvon='$nguonvon',phuongthuctt='$phuongthuctt',diadiem='$diadiem'".
		",dexuat='$dexuat',donvitinh='$donvitinh',ghichu='$ghichu' where TTcap = $id";
