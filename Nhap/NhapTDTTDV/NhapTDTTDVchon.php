...
$(document).on("click", ".open-AddBookDialog", function (e) {	
    ...
    $(_self.attr('href')).modal('show');		
		...
		<hr />
		<h3>Quản lý thay đổi thông tin đơn vị</h3>		
		<?php
		...
		function doingay1($ngay)
{
	...
	if (isset($_POST['chinhsuatss'])) 
	{
   		if ($_POST['NTTD'] != "")
		{			
			$ngaythaydoi = doingay1($_POST['NTTD']);
			$sqlin = "Update tblthaydoidv Set NTTD='$ngaythaydoi',TTTD='$_POST[TTTD]',LDTD='$_POST[LDTD]',GCTD='$_POST[GCTD]' WHERE ttthaydoi = ".$_POST['bookId'];
		...
		<table style="line-height: 0;" class="stripe row-border order-column" id="table-2">
			<thead>
				<tr>
					<th>STT</th>
					<th>Ngày thay đổi</th>
					<th>Chỉ tiêu thay đổi</th>
					<th>Thông tin kê khai</th>
...
$ab = $ma[0];
$sql = "select ttthaydoi,NTTD,CTTD,TTKK,TTTD,LDTD,GCTD from tblthaydoidv Where madonvi = '$ab'";
...
if(mysqli_num_rows($kq)>0){
	while($row=mysqli_fetch_array($kq)){		
		$nt = doingay($row[1]);	
$tatca = $row[0].">".$nt.">".$row[2].">".$row[3].">".$row[4].">".$row[5].">".$row[6];		
		$i = $i + 1;
            echo"<tr>";
                echo"<td>$row[0]</td>";
                echo"<td>$nt</td>";               
                ...
                echo"<td>
						<a data-id='$tatca' title='Add this item' class='open-AddBookDialog btn btn-blue ' href='#addBookDialog'>
							Sửa</a>
						<a href=\"xoa.php?id=$row[0]\" onclick=\"return confirm('Bạn muốn xóa thay đổi thông tin đơn vị này?')\" class='btn btn-red '>
	 ...
	 if ($i == 0)
	 {
		$sqlchitieu = "select noidung from tblchitieudv";
		$kqchitieu = mysqli_query($con,$sqlchitieu);
		while($rowct=mysqli_fetch_array($kqchitieu))
		{	
			$sqlin = "Insert into tblthaydoidv(CTTD,madonvi) values('$rowct[0]','$ab')";
...
$tatca = $row[0].">".$nt.">".$row[2].">".$row[3].">".$row[4].">".$row[5].">".$row[6];				
				...
				echo"<td>$row[6]</td>";				
				echo"<td>
						<a data-id='$tatca' title='Add this item' class='open-AddBookDialog open-AddBookDialog btn btn-blue ' href='#addBookDialog'>
							Sửa</a>
						<a href=\"xoa.php?id=$row[0]\" onclick=\"return confirm('Bạn muốn xóa đề nghị trang cấp này?')\" class='btn btn-red '>
							<i class='entypo-cancel'></i>
							Xóa
						</a>							
						</td>";
		...
		<div class="modal-dialog">
			<div class="modal-content">				
				...
				<div class="modal-body">				
						<form role='form' method='POST' action = 'NhapTDTTDVchon.php' class='form-horizontal form-groups-bordered'>	
							<div class="form-group">
								...
								<div class="col-sm-8">
									<input data-inputmask="'alias': 'date'" name="NTTD" type="text" class="form-control" id="NTTD">
								</div>							
								<label for="field-2" class="col-sm-3 control-label">Chỉ tiêu thay đổi:</label>
								<div class="col-sm-8">
									<input name="CTTD" type="text" class="form-control" id="CTTD">
								</div>							
								<label for="field-3" class="col-sm-3 control-label">Thông tin kê khai:</label>								
								<div class="col-sm-8">									
									<input name="TTKK" type="text" class="form-control" id="TTKK">									
								</div>	
								<label for="field-4" class="col-sm-3 control-label">Thông tin thay đổi:</label>
								<div class="col-sm-8">
									<input name="TTTD" type="text" class="form-control" id="TTTD">
								</div>
								<label for="field-4" class="col-sm-3 control-label">Lý do thay đổi:</label>
								<div class="col-sm-8">
									<input name="LDTD" type="text" class="form-control" id="LDTD">
								</div>							
								<label for="field-5" class="col-sm-3 control-label">Chi chú:</label>
								<div class="col-sm-8">
									<input name="GCTD" type="text" class="form-control" id="GCTD">
								</div>								
							</div>
