...
<link rel="stylesheet" type="text/css" href="/assets/News/dataTables.fixedColumns.css">
	...
	<script type="text/javascript" language="javascript" class="init">
		...
		<hr />
		<h3>Danh sách tài sản thay đổi thông tin</h3>	
<?php		
		echo "<a href='Danhsachdtdtt.php' class='btn btn-success'>
			Tạo tài sản thay đổi thông tin
		</a>";
...
$ab = $ma[0];
$sql = "select tblqlts.TTQLTS,tblqlts.chitiethinhthai,tblqlts.mataisan,tblqlts.tentaisan,tblqlts.tenchitiet,tblqlts.ngaysudung,tblqlts.noidung,tblqlts.DVT,tblqlts.DTKV,tblqlts.DTXD,tblqlts.ngansach,tblqlts.nguonkhac";
$sql = $sql . " from tblqlts inner join tblthaydoi on tblqlts.TTQLTS = tblthaydoi.ttts Where tblqlts.madonvi = '$ab'";
$sql = $sql . " Group by tblqlts.TTQLTS,tblqlts.chitiethinhthai,tblqlts.mataisan,tblqlts.tentaisan,tblqlts.tenchitiet,tblqlts.ngaysudung,tblqlts.noidung,tblqlts.DVT,tblqlts.DTKV,tblqlts.DTXD,tblqlts.ngansach,tblqlts.nguonkhac";
$kq = mysqli_query($con,$sql);  
$sl=0;$ng=0;
if(mysqli_num_rows($kq)>0){
	while($row=mysqli_fetch_array($kq)){
		$sl=$row[8] + $row[9];$ng = $row[10]+$row[11]; 
            echo"<tr>";
                echo"<td>$row[0]</td>";
                echo"<td>$row[1]</td>";               
                echo"<td>$row[2]</td>";
				...
				echo"<td>$row[6]</td>";
				echo"<td>$row[7]</td>";
				echo"<td>".dinhdangso($sl)."</td>";
				echo"<td>".dinhdangso($ng)."</td>";
                echo"<td>
						<a href=\"NhapTDTTTSchon.php?id=$row[0]\" class='open-AddBookDialog btn btn-blue '>
							
							Sửa
						</a>
						<a href=\"xoatc.php?id=$row[0]\" onclick=\"return confirm('Bạn muốn xóa thay đổi thông tin tài sản này?')\" class='btn btn-red '>
