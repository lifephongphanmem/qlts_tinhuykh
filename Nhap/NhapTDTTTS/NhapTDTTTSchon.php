...
<link rel="stylesheet" type="text/css" href="/assets/News/dataTables.fixedColumns.css">
	...
	<script type="text/javascript" language="javascript" class="init">
		...
		<hr />
		<h3>Quản lý thay đổi thông tin tài sản</h3>		
		<input type="button" class="btn btn-black" value="Trở lại" onclick="<?php echo "window.location.href='Danhsachdtdtt.php'" ?>" />
<?php
		$key=$_GET["id"];		
		echo "<a href='quanhuyen.php?id=$key' class='btn btn-success'>
			Thêm mới
		</a>";
...
?>
		<table  style="line-height: 0;" class="stripe row-border order-column"  id="table-2">
			...
			<tbody>	
<?php 
$sql = "select ttthaydoi,NTTD,CTTD,TTKK,TTTD,LDTD,GCTD from tblthaydoi where ttts = '$key'";
	...
	while($row=mysqli_fetch_array($kq)){
		...
		$kethop = $key.">".$row[0];
            echo"<tr>";
				...
				echo"<td>$row[6]</td>";				
                echo"<td>
						<a href=\"sua.php?id=$kethop\" class='btn btn-info '>
							
							Sửa
						</a>
						<a href=\"xoa.php?id=$row[0]\" onclick=\"return confirm('Bạn muốn xóa đề nghị trang cấp này?')\" class='btn btn-danger btn-sm '>
