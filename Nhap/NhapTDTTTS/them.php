...
<script language='javascript'>
 $(document).ready(function() {
  $('#chitieu').change(function() {
   giatri = this.value;
   $('#sTTKK').load('ajax_ttkk.php?id_a='+giatri);
  });
 });
</script>
<?php 
...
?>
					...
					<div class="panel-body">
						<form role="form" method="POST" class="form-horizontal form-groups-bordered">
							<div class="form-group">
								...
								<div class="col-sm-5">
									<input data-inputmask="'alias': 'date'" name="NTTD" type="text" class="form-control" id="field-1">
								</div>
							</div>	
							<div class="form-group">
								<label for="field-2" class="col-sm-3 control-label">Chỉ tiêu thay đổi:</label>
								<div class="col-sm-5">
									<select name="chitieu" type="text" class="form-control" id="chitieu">
									<option value='Chọn chỉ tiêu'>Chọn chỉ tiêu</option>									
									<?php
										$sqlpl = "Select ttct,tenchitieu from tblchitiets ";
										$querypl=mysqli_query($con,$sqlpl);
										while($rowpl=mysqli_fetch_array($querypl))		
										{						
											$ten=$rowpl['tenchitieu'];	
											$id = $rowpl['ttct'].";".$key;
							...
							<div id = "sTTKK" class="form-group">
								...
								<div class="col-sm-5">										
									<input name="TTKK" type="text" class="form-control" id="TTKK1" value = "">									
								</div>								
							</div>							
							<div class="form-group">
								<label for="field-4" class="col-sm-3 control-label">Thông tin thay đổi:</label>
								<div class="col-sm-5">
									<input name="TTTD" type="text" class="form-control" id="field-4" value = "">
								</div>
							</div>							
							<div class="form-group">
								<label for="field-4" class="col-sm-3 control-label">Lý do thay đổi:</label>
								<div class="col-sm-5">
									<input name="LDTD" type="text" class="form-control" id="field-4">
								</div>
							</div>
							<div class="form-group">
								<label for="field-5" class="col-sm-3 control-label">Chi chú:</label>
								<div class="col-sm-5">
									<input name="GCTD" type="text" class="form-control" id="field-5">
								</div>
							</div>							
							<div class="form-group">
								<div class="col-sm-offset-3 col-sm-5">
									...
									<?php										
										echo "<a href='NhapTDTTTSchon.php?id=$key' class='btn btn-success'>
											Trở lại
										</a>";
									?>
								</div>
	...
	if (isset($_POST['sua']))
	{
		if ($_POST['NTTD'] != "")
		{
			$sqlct = "Select tenchitieu from tblchitiets where ttct = ".substr($_POST['chitieu'],0, strpos($_POST['chitieu'],";"));
			$queryct=mysqli_query($con,$sqlct);
			$sqlct = "";
			while($rowct=mysqli_fetch_array($queryct))		
			{		
				$sqlct = $rowct['tenchitieu'];
			}
			$ngaythaydoi = doingay($_POST['NTTD']);
			$sqlin = "Insert into tblthaydoi(ttts,NTTD,CTTD,TTKK,TTTD,LDTD,GCTD) values('$key','$ngaythaydoi','$sqlct','".$_POST['TTKK']."','".$_POST['TTTD']."','".$_POST['LDTD']."','".$_POST['GCTD']."')";			
