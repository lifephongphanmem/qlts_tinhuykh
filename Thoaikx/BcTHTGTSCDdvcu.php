...
<table cellpadding="0" cellspacing="0" border="0" style="border-width:0px;empty-cells:show;">
	...
	<tr style="vertical-align:top;">
		...
		<td></td>
		<td></td>				
		<td class="cs62AA4CC9" colspan="9" rowspan="3" style="width:366px;height:58px;line-height:18px;text-align:center;vertical-align:middle;"><nobr>(Ban&nbsp;h&#224;nh&nbsp;theo&nbsp;QĐ&nbsp;số:&nbsp;19/2006/QĐ-BTC</nobr><br/><nobr>ng&#224;y&nbsp;30/03/2006&nbsp;của&nbsp;Bộ&nbsp;trưởng&nbsp;BTC&nbsp;v&#224;&nbsp;sửa&nbsp;đổi,</nobr><br/><nobr>bổ&nbsp;sung&nbsp;theo&nbsp;th&#244;ng&nbsp;tư&nbsp;số&nbsp;185/2010/TT-BTC)</nobr></td>
	</tr>
	<tr style="vertical-align:top;">
		<td style="width:0px;height:20px;"></td>
	...
	foreach($_aDV as $_madv) {
		...
		$sqlts = $sqlts . " and tblqlts.ngaysudung <= '" . doingay($denngay) . "' order by tbldanhsachqd32.ttsx";
		...
		while ($rowts = mysqli_fetch_array($queryts)) {
			...
			$sttangtk = 0;
			$stgiamtk = 0;
			$sqltg = "Select soluong,tanggiam,sotien from tbltanggiam where madonvi = '$_madv[ma]' and year(ngaytanggiam) < " . substr($denngay, 6, 4) . " and TTQLTS = " . $rowts['TTQLTS'];
			$querytg = mysqli_query($con, $sqltg);
			while ($rowtg = mysqli_fetch_array($querytg)) {
				if ($rowtg['tanggiam'] == "Tăng") {
					$tang = $tang + $rowtg['soluong'];
					$sttang = $sttang + $rowtg['sotien'];
				} else {
					$kq = $kq + $rowtg['soluong'];
					$stgiam = $stgiam + $rowtg['sotien'];
				}
			}
			$sqltgtk = "Select soluong,tanggiam,sotien from tbltanggiam where madonvi = '$_madv[ma]' and year(ngaytanggiam) = " . substr($denngay, 6, 4) . " and TTQLTS = " . $rowts['TTQLTS'];
			$querytgtk = mysqli_query($con, $sqltgtk);
			while ($rowtgtk = mysqli_fetch_array($querytgtk)) {
				if ($rowtgtk['tanggiam'] == "Tăng") {
					$tangtk = $tangtk + $rowtgtk['soluong'];
					$sttangtk = $sttangtk + $rowtgtk['sotien'];
				} else {
					$kqtk = $kqtk + $rowtgtk['soluong'];
					$stgiamtk = $stgiamtk + $rowtgtk['sotien'];
				}
			}
			if ($rowts['DTKV'] + $rowts['DTXD'] + $tang > $kq) {
				...
				if (kieudouble($chuoi1[0]) > kieudouble($chuoi2[2])) {
					$tg[$cs][4] = 0;
					$tg[$cs][5] = 0;
				} else if (kieudouble($chuoi1[0]) == kieudouble($chuoi2[2])) {
					if (kieudouble($chuoi1[1]) >= kieudouble($chuoi2[1])) {
						$tg[$cs][4] = 0;
						$tg[$cs][5] = 0;
					} else {
						$tg[$cs][4] = $rowts['DTKV'] + $rowts['DTXD'] + $tang - $kq;
						$tg[$cs][5] = $rowts['NG'] + $sttang - $stgiam;
					}
				} else {
					$tg[$cs][4] = $rowts['DTKV'] + $rowts['DTXD'] + $tang - $kq;
					$tg[$cs][5] = $rowts['NG'] + $sttang - $stgiam;
				...
				if (kieudouble($chuoi1[0]) > kieudouble($chuoi2[2])) {
					$tg[$cs][6] = $tangtk + $rowts['DTKV'] + $rowts['DTXD'] + $tang - $kq;
					$tg[$cs][7] = $sttangtk + $rowts['NG'] + $sttang - $stgiam;
				} else if (kieudouble($chuoi1[0]) == kieudouble($chuoi2[2])) {
					if (kieudouble($chuoi1[1]) >= kieudouble($chuoi2[1])) {
						$tg[$cs][6] = $tangtk + $rowts['DTKV'] + $rowts['DTXD'] + $tang - $kq;
						$tg[$cs][7] = $sttangtk + $rowts['NG'] + $sttang - $stgiam;
				...
				$tg[$cs][8] = $kqtk;
				$tg[$cs][9] = $stgiamtk;
				$tg[$cs][10] = $rowts['DTKV'] + $rowts['DTXD'] + $tang - $kq + $tangtk - $kqtk;
				$tg[$cs][11] = $rowts['NG'] + $sttang - $stgiam + $sttangtk - $stgiamtk;
				$cs = $cs + 1;
		...
		//print $tg;
		for ($i = 0; $i < $cs; $i++) {
			$stdk = $stdk + kieudouble($tg[$i][5]);
			$sttkt = $sttkt + kieudouble($tg[$i][7]);
			$sttkg = $sttkg + kieudouble($tg[$i][9]);
			$stck = $stck + kieudouble($tg[$i][11]);
			$stt++;
			$tennhom = $tg[$i][3];
			$tents = $tg[$i][1];
			$dvt = $tg[$i][2];
			...
			if ($tennhom != $tennhomsau) {
				...
				for ($j = 0; $j < $cs; $j++) {
					if ($tg[$j][3] == $tennhom) {
						$nhomsl2 = $nhomsl2 + $tg[$j][5];
						$nhomsl4 = $nhomsl4 + $tg[$j][7];
						$nhomsl6 = $nhomsl6 + $tg[$j][9];
						$nhomsl8 = $nhomsl8 + $tg[$j][11];
					}
				}
