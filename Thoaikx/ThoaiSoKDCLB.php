...
<div class="modal-dialog">
	<div class="modal-content">
		<div class="modal-header">
			<h4 class="modal-title" style="text-align:center;"><strong>Sổ kho</strong></h4>
		</div>
			<div class="modal-body">
				<form name="thoai" onsubmit="return kt()" role="form" method="Post" class="form-horizontal">
					...
					?>
						...
						<div class="form-group" style="display: none">
							<div class="col-sm-12"><label>Nơi sử dụng: </label><select name="noisudung" class="form-control" type = "text">
							<option value=""></option>
							<?php	
								require("$_SERVER[DOCUMENT_ROOT]/Main/connect.php");
								$sql = "Select tenphongban FROM tblphongban where madonvi = '$ma[0]' Group by tenphongban";
							...
							<div class=" col-sm-12 form-group">
								<label class="col-sm-2" style="margin-top: 7px">Nhóm công cụ: </label>
							<div class="col-sm-4"><select name="tencc" id = "tencc" class="form-control" type = "text">
							<option value=""></option>
							<?php		
								$sql = "Select distinct tenchitiet,mataisan,DVT FROM tblqlcc Where madonvi like '$ma[0]%'";
								$qrsql = mysqli_query($con,$sql);
								while($row = mysqli_fetch_array($qrsql))
								{
									$val = $row['mataisan'].">".$row['tenchitiet'].">".$row['DVT'];
									$ten = $row['tenchitiet'];
									echo "<option value = '$val'> $ten</option>";
								}
							?>
							</select></div>
								<label class="col-sm-2" style="margin-top: 7px">Mã công cụ: </label>
								<div class="col-sm-4"><select name="macc" class="form-control" id = "macc" type = "text">
										<option value=""></option>
										<?php
										$sql = "Select distinct tenchitiet,mataisan,DVT,TTQLcc FROM tblqlcc Where madonvi like '$ma[0]%'";
										$qrsql = mysqli_query($con,$sql);
										while($row = mysqli_fetch_array($qrsql))
										{
											$val = $row['mataisan'].".".$row['TTQLcc'].">".$row['tenchitiet'].">".$row['DVT'].">".$row['TTQLcc'];
											$ten = $row['mataisan'].".".$row['TTQLcc']."-".$row['tenchitiet'];
											echo "<option value = '$val'> $ten</option>";
										}
										?>
									</select></div>
						...
						<!--div class="col-sm-12 form-group">
							<label class="col-sm-2" style="margin-top: 7px;">Từ ngày: </label>
							<div class="col-sm-10"><input data-inputmask="'alias': 'date'" name="nttu" class="form-control"  type="text"></div>
						</div>
						<div class="col-sm-12 form-group">
							<label class="col-sm-2" style="margin-top: 7px;">Đến ngày: </label>
							<div class="col-sm-10"><input data-inputmask="'alias': 'date'" name="ntden" class="form-control"  type="text"></div>
						</div-->
						<div class="col-sm-12 form-group">
							<label class="col-sm-2" style="margin-top: 7px;">Năm: </label>
							<div class="col-sm-10"><input name="nam" class="form-control"  type="text"></div>
							</div>
						<input type="checkbox" name="ccdc" id = "ccdc"/>
						Xuất dữ liệu ra file excel
						<div class="form-group">
						<div class="col-sm-8">
							<input name="MADV" type="text" class="form-control" style = "Display:none;" value = "<?php echo $madv; ?>">
							<input name="tendv" type="text" class="form-control" style = "Display:none;" value = "<?php echo $ma[2]; ?>">
...
function kt()
{
   var frm = window.document.thoai;
	if(frm.tencc.value=='' && frm.macc.value=='')
	{
		alert('Xin vui lòng chọn tên hoặc mã công cụ tạo báo cáo !');
		document.forms['thoai'].tencc.focus();
		return false;
	}
	else
		return true;
