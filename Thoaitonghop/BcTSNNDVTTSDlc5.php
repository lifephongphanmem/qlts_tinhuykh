...
require ("$_SERVER[DOCUMENT_ROOT]/thoaikx/general.php");
	$ts =  array(array("A","B","C","D","E","F","G","H",0,0,0,0,0,0));
	$cs=0;
	$sql = "select count(tblqlts.TTQLTS) as s,tbldanhsachqd32.bac1,tbldanhsachqd32.bac2,tbldanhsachqd32.bac3,tbldanhsachqd32.bac4,Sum(tblqlts.DTKV+tblqlts.TDTSXD) as sl,Sum(tblqlts.ngansach) as ns,Sum(tblqlts.nguonkhac) as nk,Sum(tblqlts.ngansach+tblqlts.nguonkhac) as ng";
	$sql = $sql.",if(tbldanhsachqd32.bac5 = '',tblqlts.tenchitiet,tbldanhsachqd32.bac5) as b5";
	$sql = $sql.",if(tbldanhsachqd32.bac5 = '',year(tblqlts.ngaysudung),'') as nsd";
	...
	$sql = $sql." Group by tbldanhsachqd32.bac1,tbldanhsachqd32.bac2,tbldanhsachqd32.bac3,tbldanhsachqd32.bac4,if(tbldanhsachqd32.bac5 = '',tblqlts.tenchitiet,tbldanhsachqd32.bac5)";
	$sql = $sql.",if(tbldanhsachqd32.bac5 = '',year(tblqlts.ngaysudung),'')";
	//echo $sql;
	...
	while($rowct = mysqli_fetch_array($queryct)){
		...
		if (strpos($rowct['b5'],">") >=0)
			$sqlhm = $sqlhm." Where tbldanhsachqd32.bac1 = '".$rowct['bac1']."' and tbldanhsachqd32.bac2 = '".$rowct['bac2']."' and tbldanhsachqd32.bac3 = '".$rowct['bac3']."' and tbldanhsachqd32.bac4 = '".$rowct['bac4']."' and tblqlts.tenchitiet = '".str_replace(">","",$rowct['b5'])."'";
		else
			$sqlhm = $sqlhm." Where tbldanhsachqd32.bac1 = '".$rowct['bac1']."' and tbldanhsachqd32.bac2 = '".$rowct['bac2']."' and tbldanhsachqd32.bac3 = '".$rowct['bac3']."' and tbldanhsachqd32.bac4 = '".$rowct['bac4']."' and tbldanhsachqd32.bac5 = '".$rowct['b5']."'";
		...
		$ts[$cs][11] = $rowct['ns'];
		$ts[$cs][12] = $rowct['nk'];
		$ts[$cs][13] = $rowct['ng']-$sohaomon;
		$cs = $cs + 1;		
	}
	$tennhomsau1="";$tennhom1="";$tennhomsau2="";$tennhom2="";$tennhomsau3="";$tennhom3="";$tennhomsau4="";$tennhom4="";
	for($i=0;$i<$cs;$i++)
	{
		$tennhom1 = $ts[$i][0];		
		if ($tennhom1 != $tennhomsau1)
		{
			...
			for($j=0;$j<$cs;$j++)
			{
				if ($ts[$j][0]==$tennhom1)
				{
					$t11=$t11+$ts[$j][8];$t12=$t12+$ts[$j][9];$t13=$t13+$ts[$j][10];$t14=$t14+$ts[$j][11];$t15=$t15+$ts[$j][12];$t16=$t16+$ts[$j][13];	
				}
			}
		...
		if ($tennhom2 != $tennhomsau2)
		{
			...
			for($z=0;$z<$cs;$z++)
			{
				if ($ts[$z][1]==$tennhom2)
				{
					$t21=$t21+$ts[$z][8];$t22=$t22+$ts[$z][9];$t23=$t23+$ts[$z][10];$t24=$t24+$ts[$z][11];$t25=$t25+$ts[$z][12];$t26=$t26+$ts[$z][13];	
				}
			}
		...
		if ($tennhom3 != $tennhomsau3)
		{
			...
			for($z=0;$z<$cs;$z++)
			{
				if ($ts[$z][2]==$tennhom3)
				{
					$t31=$t31+$ts[$z][8];$t32=$t32+$ts[$z][9];$t33=$t33+$ts[$z][10];$t34=$t34+$ts[$z][11];$t35=$t35+$ts[$z][12];$t36=$t36+$ts[$z][13];	
				}
			}
		...
		if ($tennhom4 != $tennhomsau4)
		{
			...
			for($z=0;$z<$cs;$z++)
			{
				if ($ts[$z][3]==$tennhom4)
				{
					$t41=$t41+$ts[$z][8];$t42=$t42+$ts[$z][9];$t43=$t43+$ts[$z][10];$t44=$t44+$ts[$z][11];$t45=$t45+$ts[$z][12];$t46=$t46+$ts[$z][13];	
				}
			}
