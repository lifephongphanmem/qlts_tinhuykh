...
<script>
$(document).ready(function() {
  $('#ploai').change(function() {   
	if ($('#ploai').val() == 'Tổng hợp chung')
	{
		a = "ThTSNNlc.php>"+$('#khoi').val()+">"+$('#nttu').val();
		window.location.assign('ThoaiThTSNNCTlc.php?id='+a);	
	}
	else
	{
		a = "ThTSNNCTlc.php>"+$('#khoi').val()+">"+$('#nttu').val();
		window.location.assign('ThoaiThTSNNCTlc.php?id='+a);
	}
  });
 });
</script>
<?php $pl = explode(">",$_GET['id']);?>
		<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<h4 class="modal-title" style="text-align:center;"><strong>Tổng hợp tài sản nhà nước</strong></h4>
			</div>
				<div class="modal-body">
					<form name="thoai" onsubmit="return kt()" role="form" method="Post" class="form-horizontal" action="<?php echo $pl[0]; ?>">
					    <div class="form-group">
							<div class="col-sm-6">
								<label>Khối đơn vị: </label>
								<select  name="khoi" class="form-control" id="khoi"  type="text">
								<?php
								if ($pl[1] == "Huyện")
								{
									echo "<option selected = 'Selected' value = 'Huyện'>Huyện</option>";									
									echo "<option value = 'Tỉnh'>Tỉnh</option>";
								}
								else
								{
									echo "<option selected = 'Selected' value = 'Tỉnh'>Tỉnh</option>";
									echo "<option value = 'Huyện'>Huyện</option>";
								}
								?>
								</select>
							</div>
							<div class="col-sm-6">
								...
								<select  name="ploai" class="form-control" id="ploai"  type="text">									
									<?php
									if ($pl[0]=="ThTSNNlc.php")
									{
										echo "<option selected = 'Selected' value = 'Tổng hợp chung'>Tổng hợp chung</option>";
										echo "<option value = 'Chi tiết theo từng đơn vị'>Chi tiết theo từng đơn vị</option>";
										echo "<option value = ''>Chọn phân loại</option>";
									}
									else if ($pl[0]=="ThTSNNCTlc.php")
									{
										echo "<option value = ''>Chọn phân loại</option>";
										echo "<option value = 'Tổng hợp chung'>Tổng hợp chung</option>";
										echo "<option selected = 'Selected' value = 'Chi tiết theo từng đơn vị'>Chi tiết theo từng đơn vị</option>";
									}
									else
									{
										echo "<option selected = 'Selected' value = ''>Chọn phân loại</option>";
										echo "<option value = 'Tổng hợp chung'>Tổng hợp chung</option>";
										echo "<option value = 'Chi tiết theo từng đơn vị'>Chi tiết theo từng đơn vị</option>";
									}
									?>
						...
						<div class="form-group">
							<div class="col-sm-12"><label>Ngày báo cáo: </label><input data-inputmask="'alias': 'date'" name="nttu" class="form-control"  type="text" value = "<?php echo $pl[2]; ?>" ></div>
						</div>	
						<div class="form-group">	
						<div class="col-sm-8">
...
function kt()
{
   ...
   else if(frm.khoi.value=='')
   {
      alert('Xin vui lòng nhập khối kết xuất !');
      document.forms['thoai'].khoi.focus();            
      return false;
   }
   else                        
      return true;    
