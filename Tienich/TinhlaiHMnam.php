						...
						<form role="form" method="POST" class="form-horizontal form-groups-bordered">
							...
							<div class="form-group" style="text-align:center;">								
								<div id="HM" class="col-sm-offset-3 col-sm-8">
									<button type="submit" name="sua" class="btn btn-success">Tính lại HM</button>
...
$pthm32 = 0;
$pthm162 = 0;
function nguyengia ($TTQLTS,$val,$namhm)
{
	$kq =0;$ngansach =0;$nguonkhac = 0;
	global $denngay;
	global $con;
	$sql = "Select ngansach,nguonkhac,ngansachcu,nguonkhaccu,tinhlaihm from tblqlts where TTQLTS = ".$TTQLTS;
	$qrsql = mysqli_query($con,$sql);
	while($row = mysqli_fetch_array($qrsql))
	{
	    if($row["tinhlaihm"] == 1)
	    {
	        $ngansach = $row["ngansachcu"];
	        $nguonkhac = $row["nguonkhaccu"];
	    }
	    else
	    {
	...
	$sql = "Select nguonkhac,ngansach,tanggiam from tbltanggiam where YEAR(ngaytanggiam) <= '".$namhm."' and TTQLTS = ".$TTQLTS;
	$qrsql = mysqli_query($con,$sql);
	while($row = mysqli_fetch_array($qrsql))
	{
		$ngansach += $row["ngansach"];
...
function sohmnam($nguyengia,$TTQLTS,$namhm,$namsd)
{
    global $pthm32;
    global $pthm162;
    global $con;
    $tanggiam = 0;
    $sql = "Select sotien,tanggiam from tbltanggiam where YEAR(ngaytanggiam) BETWEEN '".$namsd."' and '".$namhm."' and TTQLTS = ". $TTQLTS;
    $qrsql = mysqli_query($con,$sql);
    while($row = mysqli_fetch_array($qrsql))
    {
    ...
    if($namhm < 2015)
        $haomonnam = ($nguyengia+$tanggiam)*$pthm32/100;
    else
        $haomonnam = ($nguyengia+$tanggiam)*$pthm162/100;
    return $haomonnam;
}
function soduhm($nguyengia,$TTQLTS,$namhm,$namsd)
{
    global $pthm32;
    global $pthm162;
    global $con;
    ...
    if($namsd <2013)
        $namtinhhm = $namsd +1;
    for ($i = $namtinhhm; $i < $namhm; $i++)
    {
        $tanggiam = 0;
        $sql = "Select sotien,tanggiam from tbltanggiam where YEAR(ngaytanggiam) BETWEEN '".$namsd."' and '".$i."' and TTQLTS = ". $TTQLTS;
        $qrsql = mysqli_query($con,$sql);
        while($row = mysqli_fetch_array($qrsql))
        {
            if($row["tanggiam"] == "Tăng")
...
function check($TTQLTS,$namhm)
{
    $kq = false;
    global $con;
    $sql = "Select TThaomon from tblhaomon where namhaomon = '".$namhm."' and TTTS = ".$TTQLTS;
    $qrsql = mysqli_query($con,$sql);
...
if (isset($_POST['sua']))
{
	...
	$sql = "Select TTQLTS,YEAR(ngaysudung) as namsd,(ngansach+nguonkhac) as NG,phantram,phantram32,ngansach,nguonkhac,tinhlaihm,".
		" ngansachcu+nguonkhaccu as NGC from tblqlts Where madonvi = '$ma[0]' and YEAR(ngaysudung) <= '$namhm' and phantram > 0";
	$hmnam = 0;$sodu=0;
	$kq = mysqli_query($con,$sql);
	while($row = mysqli_fetch_array($kq)){
	    $pthm162 = $row["phantram"];
	    $pthm32 = $row["phantram32"];
	    $ngansachcu = $row["ngansach"];
	    $nguonkhaccu = $row["nguonkhac"];
	    $ngansach  = nguyengia($row["TTQLTS"],"ngansach",$namhm);
		$nguonkhac = nguyengia($row["TTQLTS"],"nguonkhac",$namhm);
		if($row["tinhlaihm"] <=0 ) {
			$hmnam = sohmnam($row["NG"], $row["TTQLTS"], $namhm, $row["namsd"]);
			$sodu = soduhm($row["NG"], $row["TTQLTS"], $namhm, $row["namsd"]);
		}
	    else {
			$hmnam = sohmnam($row["NGC"], $row["TTQLTS"], $namhm, $row["namsd"]);
			$sodu = soduhm($row["NGC"], $row["TTQLTS"], $namhm, $row["namsd"]);
		}
		$sohaomon = $hmnam+$sodu;
		$conlai = $ngansach + $nguonkhac - $sohaomon;
		if(check($row["TTQLTS"],$namhm)) {
			$sql = "update tblqlts set sohaomon = $sohaomon,conlai = $conlai,ngansach = $ngansach, nguonkhac = $nguonkhac" .
				($row["tinhlaihm"] <= 0 ? ",ngansachcu = '$ngansachcu',nguonkhaccu = '$nguonkhaccu',tinhlaihm = '1'" : "") . " where TTQLTS = " . $row["TTQLTS"];
			$qrsql = mysqli_query($con, $sql);
		}
		$sql = "update tblhaomon set sodu=$sodu,sotien = $hmnam where namhaomon = $namhm and TTQLTS = ".$row["TTQLTS"] ;
		$qrsql = mysqli_query($con,$sql);
	}
}
}
